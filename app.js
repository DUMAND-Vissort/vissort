...
// ==================== app.js: Конец части 1 из 4 ====================
// ==================== app.js: Часть 2A из 4 ====================
...
// ==================== ГЕНЕРАТОР SVG: сетка одиночного стимула ====================
...
// ==================== app.js: Конец части 2A ====================
// ==================== app.js: Часть 2B из 4 ====================
...
async function saveNodeAsTemplate(nodeId) {
    ...
    // Клонируем узел без координат и служебных полей
    ...
    try {
        const saved = await window.Data.saveTemplate({
            name,
            description,
            kind: 'node',
            payload
        ...
        // Если открыта модалка заготовок — обновим
...
async function applyTemplateToCanvas(templateId, position) {
    ...
    if (!tpl) { alert('Заготовка не найдена'); return; }
    ...
    if (tpl.kind === 'group' && tpl.payload?.nodes) {
        // Импорт группы узлов с новыми id
        const idMap = new Map();
...
async function loadTemplatesFolder() {
    if (!HAS_FS_ACCESS) {
        if (templatesFolderInput) { templatesFolderInput.value = ''; templatesFolderInput.click(); }
        return;
    }
    try {
...
async function importTemplatesFromFolder(handle) {
    ...
    if (!ok) { alert('Нет доступа к папке заготовок'); return; }
    const files = await window.Data.scanFolder(handle, '.json');
    if (files.length === 0) { alert('В папке заготовок нет .json файлов'); return; }
...
async function importTemplatesFromFileList(fileList) {
    ...
    if (!files.length) { alert('Не выбрано ни одного .json'); return; }
...
async function renderTemplatesList(filter = '') {
    ...
    const filtered = q ? list.filter(t => (t.name || '').toLowerCase().includes(q) || (t.description || '').toLowerCase().includes(q)) : list;
    container.innerHTML = '';
    if (filtered.length === 0) {
        container.innerHTML = '<div style="padding:20px;color:#666;text-align:center;font-style:italic;">Пока нет заготовок. Сохраните узел через 💾 или импортируйте папку.</div>';
        return;
    }
    filtered.forEach(tpl => {
        const item = document.createElement('div');
        item.style.cssText = 'padding:10px 12px;background:#16161a;border:1px solid #2a2a3a;border-radius:6px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:flex-start;gap:10px;';
        const kindLabel = tpl.kind === 'group' ? `👥 группа (${(tpl.payload?.nodes || []).length} узлов)` : '🎯 узел';
...
function openTemplatesModal() {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="modal-content" style="width:560px;">
                ...
                <p style="font-size:12px;color:#94a3b8;margin:0 0 10px;">
                    Готовые узлы и группы узлов. Сохраните узел через кнопку 💾 на узле — он появится здесь.
                </p>
        ...
        document.getElementById('templates-close').addEventListener('click', () => modal.style.display = 'none');
        modal.addEventListener('click', e => { if (e.target === modal) modal.style.display = 'none'; });
        document.getElementById('templates-search').addEventListener('input', e => renderTemplatesList(e.target.value));
...
async function exportAllTemplates() {
    const list = await window.Data.getTemplatesLocal();
    if (!list.length) { alert('Нет заготовок для экспорта'); return; }
...
// ==================== app.js: Конец части 2B ====================
// ==================== app.js: Часть 3A из 4 ====================
...
function openBookPicker() {
    ...
    const f = document.getElementById('book-picker-file-name'); if (f) f.textContent = '';
    const i = document.getElementById('book-picker-file'); if (i) i.value = '';
...
function applyBookSelection(bookId) {
    ...
    if (!bk) { alert('Книга не найдена'); return; }
    node.bookId = bookId;
...
function detachBookFromActiveNode() {
    const node = getNode(activeNodeId);
    if (!node || node.nodeType !== 'READING') return;
    node.bookId = null; node.bookName = '';
    closeBookPicker();
    requestRenderGraph();
    updateInspector();
}
...
function applyReadingFontForNode(node) {
    ...
    readingContentEl.style.fontFamily = safe.toLowerCase() === 'sivtsev' ? `'Sivtsev', 'Segoe UI', sans-serif` : `'${safe}', 'Segoe UI', Tahoma, sans-serif`;
...
function setupReadingColumns() {
    ...
    if (vw <= 0) return;
    const sp = 80, tw = Math.max(200, vw - sp);
...
function toggleReadingPause() {
    ...
    if (readingPaused) { readingContentEl.style.opacity = '0'; if (b) b.textContent = '▶ Чтение'; }
    else { readingContentEl.style.opacity = '1'; if (b) b.textContent = '⏸ Пауза'; }
}
function saveReadingBookmark() {
    if (!window._currentReadingBookId) { alert('Нет книги'); return; }
    localStorage.setItem(bookmarkKeyFor(window._currentReadingBookId), JSON.stringify({ page: readingPage, updatedAt: new Date().toISOString() }));
...
function openReadingBookmark() {
    ...
    if (!s) { alert('Закладка не найдена'); return; }
    try { scrollReadingToPage(JSON.parse(s).page); } catch (e) { alert('Ошибка'); }
}
function changeAcuityByStep(delta) {
    ...
    if (readingContentEl) readingContentEl.style.fontSize = acuityToFontSizePx(node.readingAcuity, d, node.readingPPI || screenPPI) + 'px';
    setTimeout(() => { readingTotalPages = calcReadingTotalPages(); scrollReadingToPage(0); }, 60);
}
function updateReadingFontIndicator() {
    const f = document.getElementById('reading-font-input'); if (f) f.value = readingFontFamily;
    const b = document.getElementById('reading-bold-toggle'); if (b) b.classList.toggle('active', readingFontWeight === 'bold');
}
...
function buildPhysiologicalPhases(step) {
    ...
    const sel = [];
    for (const p of PHYSIOLOGICAL_PHASES) { const r = p.diopters / sd; if (Math.abs(r - Math.round(r)) < 1e-6) sel.push(p); }
    ...
    for (let i = 0; i < sel.length - 1; i++) {
        ...
        ph.push({ from: { r: a.rgb[0], g: a.rgb[1], b: a.rgb[2] }, to: { r: b.rgb[0], g: b.rgb[1], b: b.rgb[2] }, metaFrom: a, metaTo: b });
    }
    return ph;
}
function buildDynamicPhases(p) {
    const m = p.dynamicMode || 'simple', rev = p.dynamicReverse === true;
    let base;
    if (m === 'physiological') base = buildPhysiologicalPhases(p.dynamicStep || 0.1);
        ...
        const A = p.dynamicStartColor || { r: 255, g: 0, b: 0 }, B = p.dynamicEndColor || { r: 0, g: 0, b: 255 };
        base = (p.dynamicMidEnabled && p.dynamicMidColor) ? [{ from: A, to: p.dynamicMidColor }, { from: p.dynamicMidColor, to: B }] : [{ from: A, to: B }];
    }
    if (rev) return base.concat(base.slice().reverse().map(ph => ({ from: ph.to, to: ph.from, metaFrom: ph.metaTo, metaTo: ph.metaFrom })));
    return base;
}
function updatePhysioIndicator(meta, p) {
    const i = document.getElementById('physio-indicator');
    if (!i) return;
    if (p && p.dynamicShowLabel === false) { i.style.display = 'none'; return; }
    if (!meta) { i.style.display = 'none'; return; }
...
function startDynamicReadingBackground(p) {
    ...
    function tick(now) {
        ...
        if (t >= 1) {
            if (loop) { readingBgAnimStart += Math.floor(t) * cycleMs; el = Math.max(0, now - readingBgAnimStart); t = el / cycleMs; }
            else {
...
function stopDynamicReadingBackground() {
    if (readingBgAnimId) { cancelAnimationFrame(readingBgAnimId); readingBgAnimId = null; }
...
function applyReadingBackground(p) {
    ...
    if (m === 'split') {
        ...
        const lc = p.splitLeftColor || { r: 0, g: 0, b: 0 }, rc = p.splitRightColor || { r: 0, g: 0, b: 0 };
    ...
    if (m === 'gradient') {
        const lc = p.gradientLeftColor || { r: 255, g: 255, b: 255 }, rc = p.gradientRightColor || { r: 0, g: 0, b: 0 };
        if (p.gradientMidEnabled !== false) {
            const mc = p.gradientMidColor || { r: 204, g: 204, b: 204 }, mp = Math.max(0, Math.min(100, p.gradientMidPosition ?? 50));
        ...
        } else readingViewportEl.style.background = `linear-gradient(to right, rgb(${lc.r},${lc.g},${lc.b}) 0%, rgb(${rc.r},${rc.g},${rc.b}) 100%)`;
        return;
    }
...
function collectFontList() {
    const base = ['Sivtsev','Segoe UI','Arial','Times New Roman','Calibri','Verdana','Georgia','Tahoma','Trebuchet MS','Consolas','Courier New','Cambria','Garamond','Palatino Linotype','Comic Sans MS','Bookman Old Style','Franklin Gothic Medium','Century Gothic','Lucida Console','MS Sans Serif'];
    const seen = new Set(base.map(f => f.toLowerCase()));
    const dl = document.getElementById('reading-font-list');
    if (dl) for (const o of dl.options) { const v = o.value; if (v && !seen.has(v.toLowerCase())) { seen.add(v.toLowerCase()); base.push(v); } }
    return base;
}
function renderFontPickerList(filter) {
    const c = document.getElementById('font-picker-list'); if (!c) return;
    const fonts = collectFontList();
    const f = (filter || '').trim().toLowerCase();
    const list = f ? fonts.filter(x => x.toLowerCase().includes(f)) : fonts;
    c.innerHTML = '';
    if (list.length === 0) { c.innerHTML = '<div class="font-picker-empty">Ничего не найдено</div>'; return; }
    ...
    list.forEach(name => {
        ...
        item.addEventListener('click', () => {
            const node = getNode(window._currentReadingNodeId);
            if (node && node.nodeType === 'READING') { node.readingFontFamily = name; applyReadingFontForNode(node); requestRenderGraph(); }
            else { readingFontFamily = name; if (readingContentEl) readingContentEl.style.fontFamily = `'${name.replace(/['"]/g, '')}', 'Segoe UI', sans-serif`; }
            saveUserSettings(); closeFontPicker();
...
function openFontPicker() {
    const m = document.getElementById('font-picker-modal'); if (!m) return;
    const s = document.getElementById('font-picker-search'); if (s) s.value = '';
...
// ==================== app.js: Конец части 3A ====================
// ==================== app.js: Часть 3B из 4 ====================
...
function startPlayer() {
    if (window._pendingGeneratorMode && trainingNode && trainingNode.params) { startAutoTraining(); return; }
    if (nodes.length > 0) { playNodesSequence(); return; }
    if (trainingNode && trainingNode.params) { startAutoTraining(); return; }
...
function startAutoTraining() {
    ...
    if (tt === 'reading') { btnPlayerPause.disabled = true; startReadingModeAuto(); }
        ...
        gridX = trainingNode.params.gridX || 3; gridY = trainingNode.params.gridY || 3;
        activeCells = trainingNode.params.activeCells || [];
        cellParams = trainingNode.params.cellParams || [];
        if (activeCells.length === 0) activeCells = [{ row: 0, col: 0 }, { row: 0, col: 1 }];
...
function playNodesSequence() {
    buildPlayQueue();
    if (playQueue.length === 0) { stopPlayer(); return; }
    if (currentMode === 'nodes') switchMode('stimuli');
    playerRunning = true; isPaused = false;
    completedSeries = successfulSeries = failedSeries = 0;
    seriesCorrect = seriesIncorrect = seriesNoAnswer = 0;
...
function playNextQueuedNode() {
    ...
    if (item.connection) {
        const c = item.connection, fn = getNode(item.fromNodeId);
        if (fn) {
            if (c.inheritSize) { node.stimAcuity = fn.stimAcuity; node.endAcuity = fn.endAcuity; node.stimDistance = fn.stimDistance; node.stimPPI = fn.stimPPI; node.stimSize = getNodeComputedSize(node); }
...
function playStimulusNodeSeries(node) {
    ...
    if (node.singleGridEnabled) {
        const gx = node.singleGridX || 1, gy = node.singleGridY || 1;
        const cell = pickSingleGridCell(gx, gy, node.singleGridRandomCell !== false, node.singleGridAvoidRepeat !== false, node.singleGridFixedRow || 0, node.singleGridFixedCol || 0, node.singleGridCells || []);
    ...
    currentShowTimer = setTimeout(() => {
        ...
        else { seriesNoAnswer++; if (window.Voice) window.Voice.sayKey('timeout', { cancel: true }); }
...
function finishStimulusNodeSeries(node) {
    ...
    if (ok) {
        ...
        if (nodeAcuityCurrent < eA) nodeAcuityCurrent = Math.min(eA, Math.round((nodeAcuityCurrent + (node.acuityStep || 0.1)) * 10) / 10);
    } else {
        const sA = node.stimAcuity || 1.0;
        if (nodeAcuityCurrent > sA) nodeAcuityCurrent = Math.max(sA, Math.round((nodeAcuityCurrent - (node.acuityStep || 0.1)) * 10) / 10);
    }
    seriesCorrect = seriesIncorrect = seriesNoAnswer = seriesStep = 0;
    lastDirection = null; updateCounters();
    if (noAnswerSeriesStreak >= 3) { pauseTraining(); return; }
    if (completedSeries >= (node.seriesCount || 5)) { playIndex++; playNextQueuedNode(); return; }
...
function playReadingNode(node) {
    ...
    if (!bid) {
        if (window._currentReadingBookId && window._books[window._currentReadingBookId]) { bid = window._currentReadingBookId; inherit = true; }
        else { const l = getLastBookId(); if (l) { bid = l; inherit = true; } }
    }
    if (inherit && window._currentReadingBookId === bid) page = window._currentReadingPage || 0;
        ...
        if (s) { try { page = JSON.parse(s).page || 0; } catch (_) {} }
    }
...
function startReadingNodeTimer(sec) {
    stopReadingNodeTimer();
    const el = document.getElementById('reading-timer'); if (!el) return;
    let s = sec;
    el.textContent = `⏱ ${s} с`;
    el.style.display = 'block';
    readingNodeTimerInterval = setInterval(() => {
        s--;
        if (s <= 0) { stopReadingNodeTimer(); return; }
        el.textContent = `⏱ ${s} с`;
    }, 1000);
}
function stopReadingNodeTimer() {
    if (readingNodeTimerInterval) { clearInterval(readingNodeTimerInterval); readingNodeTimerInterval = null; }
...
function showDirectionComparison() {
    ...
    const d1 = randomDirection(), d2 = randomDirection();
...
function showFindSameComparison() {
    const pc = Math.max(1, Math.min(20, parseInt(currentCompareNode?.pairsCount || trainingNode?.params?.pairsCount || 2)));
    const need = pc * 2;
    if (activeCells.length < need) {
        const maxP = Math.floor(activeCells.length / 2);
        if (maxP < 1) { processComparisonAnswer(false); return; }
...
function showFindSameComparisonInternal(pc) {
    ...
    const pairs = [];
    for (let i = 0; i < pc; i++) pairs.push({ cells: [chosen[i * 2], chosen[i * 2 + 1]], direction: randomDirection(), found: false });
    ...
    responseButtons.style.display = 'none';
    document.querySelectorAll('.grid-cell').forEach(el => { el.onclick = () => handleFindSameClick(parseInt(el.dataset.index)); });
}
function handleFindSameClick(idx) {
    ...
    if (st.firstSelectedIdx === null) { st.firstSelectedIdx = idx; flashCell(idx, 'selected'); return; }
    const fi = st.firstSelectedIdx;
    if (fi === idx) { st.firstSelectedIdx = null; flashCell(idx, 'unselect'); return; }
    ...
    if (fp === sp) {
        ...
        if (st.pairs.every(p => p.found)) {
            lastResponse = { answered: true, isCorrect: true, reactionTimeMs: performance.now() - responseStartTime };
...
function createCellElement(cell, params, direction, idx) {
    const cw = stimDisplay.offsetWidth / gridX, ch = stimDisplay.offsetHeight / gridY;
    const el = document.createElement('div');
...
function stopPlayer() {
    ...
    if (currentSessionId) {
        if (window.Data && window.Data.isOnline()) {
            // Итоговая статистика в облако — через Data
            // (таблица training_sessions остаётся, но прямой вызов supabase убран;
            //  при желании можно добавить отдельный метод Data.saveSessionSummary)
        }
...
// ==================== app.js: Конец части 3B ====================
// ==================== app.js: Часть 4A из 4 ====================
...
function ensureSingleGridGeneratorUI() {
    ...
    block.style.cssText = 'margin-top:10px;padding:8px;background:#0a1a10;border:1px solid #22c55e;border-radius:4px;';
    block.innerHTML = `
...
function ensureDistanceGeneratorUI() {
    ...
    block.style.cssText = 'margin-top:12px;padding:8px;background:#0a1020;border:1px solid #0ea5e9;border-radius:4px;';
    block.innerHTML = `
        <label style="color:#38bdf8;font-weight:bold;display:block;margin-bottom:6px;">📏 Дистанции</label>
        <label>Общая (стимулы), м</label>
        <input type="number" id="gen-general-distance" value="${generalDistance}" min="0.1" max="20" step="0.1">
        <label>Чтение, м</label>
        <input type="number" id="gen-reading-distance" value="${readingDistance}" min="0.1" max="20" step="0.1">
...
function ensureBlinkGeneratorUI() {
    ...
    block.style.cssText = 'margin-top:12px;padding:8px;background:#0a1a1a;border:1px solid #14b8a6;border-radius:4px;';
    block.innerHTML = `
    ...
    firstCol.appendChild(block);
    document.getElementById('gen-blink-enabled').addEventListener('change', e => { blinkEnabled = e.target.checked; saveUserSettings(); });
    document.getElementById('gen-blink-min-rate').addEventListener('change', e => { const v = parseFloat(e.target.value); if (!isNaN(v) && v >= 0 && v <= 60) { blinkMinRate = v; saveUserSettings(); } });
    document.getElementById('gen-blink-window').addEventListener('change', e => { const v = parseFloat(e.target.value); if (!isNaN(v) && v >= 5 && v <= 120) { blinkWindowSec = v; saveUserSettings(); } });
    document.getElementById('gen-blink-lock-show').addEventListener('change', e => { blinkLockShow = e.target.checked; saveUserSettings(); });
}
...
function renderGridPreview() {
    ...
    for (let row = 0; row < gY; row++) for (let col = 0; col < gX; col++) {
        const el = document.createElement('div');
        el.className = 'grid-cell';
        el.dataset.row = row;
        el.dataset.col = col;
...
function collectCompareParams() {
    ...
    document.querySelectorAll('#cell-params-container > div').forEach(div => {
        ...
        cp.push({ size: acuityToSizePx(V, d, ppi), acuity: V, distance: d, ppi, stimR: sc.r, stimG: sc.g, stimB: sc.b, bgR: bc.r, bgG: bc.g, bgB: bc.b, duration: dur });
    });
...
function buildModeParamsFromUI() {
    ...
    if (gdEl) { const v = parseFloat(gdEl.value); if (!isNaN(v) && v > 0) { generalDistance = v; saveUserSettings(); } }
    const rdEl = document.getElementById('gen-reading-distance');
    if (rdEl) { const v = parseFloat(rdEl.value); if (!isNaN(v) && v > 0) { readingDistance = v; saveUserSettings(); } }
    const tiEl = document.getElementById('gen-tol-inc');
    if (tiEl) { const v = parseFloat(tiEl.value); if (!isNaN(v) && v >= 0 && v <= 100) { distanceToleranceIncreasePct = v; saveUserSettings(); } }
    const tdEl = document.getElementById('gen-tol-dec');
    if (tdEl) { const v = parseFloat(tdEl.value); if (!isNaN(v) && v >= 0 && v <= 100) { distanceToleranceDecreasePct = v; saveUserSettings(); } }
    const ttEl = document.getElementById('gen-tol-timeout');
    if (ttEl) { const v = parseFloat(ttEl.value); if (!isNaN(v) && v >= 0 && v <= 60) { distanceRestoreTimeoutSec = v; saveUserSettings(); } }
    const p = {
...
// ==================== app.js: Конец части 4A ====================
// ==================== app.js: Часть 4B из 4 (финальная) ====================
...
async function scanScenariosFolder() {
    ...
    if (!ok) { console.warn('[scenarios] нет доступа к папке'); return; }
    ...
    for (const f of files) {
        try {
            ...
            const record = {
                ...
                created_by: window.Data.getCurrentUserId(),
                _dirty: true,
                _syncedAt: null,
                _localFile: f.name
            };
    ...
    if (imported > 0) {
        console.log(`[scenarios] импортировано из папки: ${imported}`);
    }
}
...
async function saveGraph() {
    ...
    try {
        ...
        // 1. Пишем в папку, если она выбрана
        if (HAS_FS_ACCESS && selectedFolderHandle) {
            try {
                ...
                if (ok) {
                    ...
                    if (!fileName) {
                        const ans = prompt('Имя файла сценария (без пути):', 'scenario_' + new Date().toISOString().slice(0, 10) + '.json');
                        if (!ans) return;
        ...
        // 2. Всегда сохраняем в Data (IndexedDB + очередь синхронизации)
        const record = {
    ...
    reader.onload = e => {
        try {
            ...
            if (data.distances) {
                ...
                if (typeof data.distances.incTol === 'number') distanceToleranceIncreasePct = data.distances.incTol;
                if (typeof data.distances.decTol === 'number') distanceToleranceDecreasePct = data.distances.decTol;
                if (typeof data.distances.timeout === 'number') distanceRestoreTimeoutSec = data.distances.timeout;
            }
            if (data.blink) {
            ...
            connections.forEach(c => {
                ...
                delete c.inheritBook; delete c.inheritPosition;
            });
            nodes.forEach(node => {
                ...
                if (node.nodeType === 'COMPARE') {
                    ...
                    if (!Array.isArray(node.activeCells) || node.activeCells.length < 2) node.activeCells = [{ row: 0, col: 0 }, { row: 0, col: 1 }];
                    if (!Array.isArray(node.cellParams)) node.cellParams = [];
                    while (node.cellParams.length < node.activeCells.length) node.cellParams.push(defaultCompareCellParams());
                    if (node.seriesCount === undefined) node.seriesCount = 5;
                    if (node.seriesSize === undefined) node.seriesSize = 6;
                    if (node.seriesThreshold === undefined) node.seriesThreshold = 4;
    ...
    try {
        ...
        list.querySelectorAll('.btn-load-scenario').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                if (!sc) { alert('Сценарий не найден'); return; }
...
async function assignScenario() {
    ...
    if (!u || !s) { alert('Выберите пользователя и сценарий'); return; }
...
function addBlock() {
    const type = blockSelect.value, name = blockNameInput.value.trim() || 'Блок';
    let p = {};
...
function generateSequenceFromBlocks() {
    if (blockList.length === 0) { alert('Список пуст'); return; }
    nodes = []; connections = []; activeNodeId = null;
    inspectorEl.style.display = 'none';
    updateInspector();
    let prevId = null;
    blockList.forEach((b, i) => {
        const x = 100 + i * 300, y = 100;
        const nid = createNewNode('STIMULUS', x, y);
        const n = getNode(nid);
        n.name = b.name || 'Блок ' + (i+1);
        n.duration = b.params.duration || 1000;
        if (prevId) connections.push({ fromId: prevId, toId: nid, isLoop: false, loopMode: 'TIME', loopLimit: 1, lifeTime: 0, initiateByAnswer: true, condition: 'none', inheritSize: false, inheritSpeed: false });
        prevId = nid;
    });
    activeNodeId = nodes.length > 0 ? nodes[0].id : null;
    if (activeNodeId) { inspectorEl.style.display = 'block'; updateInspector(); }
    requestRenderGraph();
    alert('Последовательность создана.');
}

// ==================== AUTH (обёртка над Data + supabase-js) ====================
function initSupabase() {
    // supabaseClient нужен только для auth (Data использует свой REST-клиент)
    supabaseClient = window.supabase.createClient(
        'https://hzvypwdpdhsjzcacl.mb.supabase.co'.replace('hzvypwdpdhsjzcacl.mb', 'hzvypwdpdhsjzaclxmbm'),
...
function init() {
    ...
    document.getElementById('reading-font-input')?.addEventListener('change', e => {
        ...
        if (!n) return;
        const node = getNode(window._currentReadingNodeId);
        if (node && node.nodeType === 'READING') { node.readingFontFamily = n; applyReadingFontForNode(node); requestRenderGraph(); }
        else { readingFontFamily = n; if (readingContentEl) readingContentEl.style.fontFamily = `'${n.replace(/['"]/g, '')}', 'Segoe UI', sans-serif`; }
        saveUserSettings();
        setTimeout(() => { setupReadingColumns(); readingTotalPages = calcReadingTotalPages(); scrollReadingToPage(0); }, 60);
    });
    ...
    document.getElementById('reading-font-sivtsev')?.addEventListener('click', () => {
        const node = getNode(window._currentReadingNodeId);
        if (node && node.nodeType === 'READING') { node.readingFontFamily = 'Sivtsev'; applyReadingFontForNode(node); requestRenderGraph(); }
        else { readingFontFamily = 'Sivtsev'; if (readingContentEl) readingContentEl.style.fontFamily = `'Sivtsev', 'Segoe UI', sans-serif`; }
        const i = document.getElementById('reading-font-input'); if (i) i.value = 'Sivtsev';
        saveUserSettings();
        setTimeout(() => { setupReadingColumns(); readingTotalPages = calcReadingTotalPages(); scrollReadingToPage(0); }, 60);
    });
    ...
    if (fontFileBtn && fontFileInput) {
        ...
        fontFileInput.addEventListener('change', async e => {
            ...
            try {
                ...
                const familyName = f.name.replace(/\.[^.]+$/, '').replace(/[^a-zA-Z0-9\u0400-\u04FF_ -]/g, '_') || 'CustomFont';
                const font = new FontFace(familyName, buf);
                await font.load();
                document.fonts.add(font);
                const node = getNode(window._currentReadingNodeId);
                if (node && node.nodeType === 'READING') { node.readingFontFamily = familyName; applyReadingFontForNode(node); requestRenderGraph(); }
                else { readingFontFamily = familyName; if (readingContentEl) readingContentEl.style.fontFamily = `'${familyName.replace(/['"]/g, '')}', 'Segoe UI', sans-serif`; }
                const i = document.getElementById('reading-font-input'); if (i) i.value = familyName;
                saveUserSettings();
                setTimeout(() => { setupReadingColumns(); readingTotalPages = calcReadingTotalPages(); scrollReadingToPage(0); }, 60);
            } catch (err) { alert('Не удалось загрузить шрифт: ' + err.message); }
            fontFileInput.value = '';
        });
    }
    document.getElementById('reading-font-system')?.addEventListener('click', async () => {
        if (!('queryLocalFonts' in window)) { alert('Браузер не поддерживает список системных шрифтов.'); return; }
        try {
            ...
            list.innerHTML = '';
            const first = document.createElement('option'); first.value = 'Sivtsev'; first.textContent = 'Сивцев'; list.appendChild(first);
            const pref = ['Segoe UI','Arial','Times New Roman','Calibri','Verdana','Georgia','Tahoma'];
            const ordered = [...pref.filter(p => fams.includes(p)), ...fams.filter(f => !pref.includes(f) && f.toLowerCase() !== 'sivtsev')];
            for (const fam of ordered) { const o = document.createElement('option'); o.value = fam; list.appendChild(o); }
            alert(`Загружено ${fams.length} шрифтов.`);
        } catch (err) { alert('Ошибка: ' + err.message); }
    });
    document.getElementById('reading-bold-toggle')?.addEventListener('click', () => {
        const node = getNode(window._currentReadingNodeId);
        if (node && node.nodeType === 'READING') { node.readingFontWeight = (node.readingFontWeight === 'bold') ? 'normal' : 'bold'; applyReadingFontForNode(node); requestRenderGraph(); }
        else { readingFontWeight = (readingFontWeight === 'bold') ? 'normal' : 'bold'; if (readingContentEl) readingContentEl.style.fontWeight = readingFontWeight; }
        const b = document.getElementById('reading-bold-toggle');
        if (b) { const isB = (node ? node.readingFontWeight : readingFontWeight) === 'bold'; b.classList.toggle('active', isB); }
        saveUserSettings();
        setTimeout(() => { setupReadingColumns(); readingTotalPages = calcReadingTotalPages(); scrollReadingToPage(0); }, 60);
    });
    ...
    if (readingFileInput) {
        readingFileInput.addEventListener('change', async e => {
            ...
            if (!(n.endsWith('.txt') || n.endsWith('.fb2') || n.endsWith('.epub'))) { alert('Поддерживаются .txt, .fb2, .epub'); readingFileInput.value = ''; return; }
    ...
    document.getElementById('gen-to-node').addEventListener('click', () => {
        ...
        if (tt === 'reading') { modeParamsToReadingNode(mp); alert('📌 Узел «Чтение» создан.'); }
        else if (tt === 'single') { modeParamsToNode(mp); alert('📌 Узел создан.'); }
    ...
    openDatabaseLegacy().then(async () => {
        await restoreFolderHandle();
        await restoreTemplatesFolderHandle();
        // Автоматически импортируем заготовки из папки, если она выбрана
        if (templatesFolderHandle) {
            try { await importTemplatesFromFolder(templatesFolderHandle); } catch (_) {}
        }
    }).catch(err => console.warn('[init] folder restore:', err));

    registerServiceWorker();
}

// Совместимость: старые вызовы openDatabase() больше не нужны — Data всё делает.
// Оставляем пустую обёртку, чтобы не падало.
function openDatabaseLegacy() {
