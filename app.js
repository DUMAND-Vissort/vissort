...
let selectedCells = [],
    singleGridX = 1,
    singleGridY = 1;
let currentCompareNode = null,
    _findSameState = null;

let readingDistance = 1,
    generalDistance = 1;
let distanceToleranceIncreasePct = 15,
    distanceToleranceDecreasePct = 10,
    distanceRestoreTimeoutSec = 3;
let _distanceBaseline = null,
    _distanceOutOfBoundsSince = 0,
    _distanceWarningKind = null,
    _distanceRecalcScheduled = false;
...
let readingViewportEl = null,
    readingContentEl = null,
    readingToolbarEl = null;
let readingPage = 0,
    readingTotalPages = 1,
    readingPaused = false;
let readingFontFamily = 'Segoe UI',
...
let trainingNode = null,
    ...
    templatesFolderHandle = null;
let currentPlayingNodeId = null,
    nodeAcuityCurrent = 1.0;
...
function safeChecked(id, defVal) {
    const el = document.getElementById(id);
...
function bookmarkKeyFor(bookId) {
    return 'vissort_bookmark_' + (window._currentScenarioKey || 'default') + '_' + bookId;
}
function getLastBookId() {
    try {
        const k = 'vissort_lastBookId_' + (window._currentScenarioKey || 'default');
        const id = localStorage.getItem(k);
        if (id && window._books && window._books[id]) return id;
    } catch (_) {}
    return null;
}
...
function loadUserSettings() {
    ...
    const it = parseFloat(localStorage.getItem(LS_KEYS.distIncTol));
    if (!isNaN(it) && it >= 0 && it <= 100) distanceToleranceIncreasePct = it;
    const dt = parseFloat(localStorage.getItem(LS_KEYS.distDecTol));
    if (!isNaN(dt) && dt >= 0 && dt <= 100) distanceToleranceDecreasePct = dt;
    const to = parseFloat(localStorage.getItem(LS_KEYS.distTimeout));
    if (!isNaN(to) && to >= 0 && to <= 60) distanceRestoreTimeoutSec = to;
    const be = localStorage.getItem(LS_KEYS.blinkEnabled);
    if (be !== null) blinkEnabled = be === 'true';
    const bt = parseFloat(localStorage.getItem(LS_KEYS.blinkThreshold));
    if (!isNaN(bt) && bt > 0.05 && bt < 0.5) blinkThreshold = bt;
    const bm = parseFloat(localStorage.getItem(LS_KEYS.blinkMinRate));
    if (!isNaN(bm) && bm >= 0 && bm <= 60) blinkMinRate = bm;
    const bw = parseFloat(localStorage.getItem(LS_KEYS.blinkWindow));
    if (!isNaN(bw) && bw >= 5 && bw <= 120) blinkWindowSec = bw;
    const bls = localStorage.getItem(LS_KEYS.blinkLockShow);
    if (bls !== null) blinkLockShow = bls === 'true';
}
...
async function parseFb2File(file) {
    ...
    bodies.forEach((body) => {
        ...
        body.querySelectorAll('title, subtitle, p, v, text-author, empty-line, poem, epigraph, cite').forEach(
            (node) => {
                if (node.tagName.toLowerCase() === 'empty-line') {
                    result.push('');
                    return;
                }
...
async function parseEpubFile(file) {
    ...
    if (spine.length === 0)
        Object.values(manifest).forEach((m) => {
            if (/html|xhtml/i.test(m.mediaType) || /\.x?html?$/i.test(m.href)) spine.push(m.href);
        });
    const result = [];
...
async function extractAllHtmlFromZip(zip) {
    const files = [];
    zip.forEach((p, e) => {
        if (!e.dir && /\.(x?html?|htm)$/i.test(p)) files.push({ p, e });
    });
    files.sort((a, b) => a.p.localeCompare(b.p));
    const result = [];
    for (const { e } of files) {
        const t = extractTextFromHtml(await e.async('string'));
        if (t) result.push(t);
    }
    return result.join('\n\n');
}
...
function estimateScenarioDurationMs() {
    ...
    function nodeDuration(node) {
        ...
        if (node.singleStimDynamicEnabled)
            perNode = Math.max(
                perNode,
                (delay1 + (node.singleStimDuration || 10000) + delay2) * seriesCount * seriesSize
            );
        if (node.singleBgDynamicEnabled)
            perNode = Math.max(
                perNode,
                (delay1 + (node.singleBgDuration || 10000) + delay2) * seriesCount * seriesSize
            );
        return perNode;
    }
...
function updateLiveDistanceIndicator() {
    ...
    if (!indicator) return;
    if (currentDistanceMeters == null || !isFinite(currentDistanceMeters)) {
...
function hideLiveDistanceIndicator() {
    const i = document.getElementById('live-distance-indicator');
...
function setDistanceBaselineIfNeeded() {
    if (_distanceBaseline == null && currentDistanceMeters != null && isFinite(currentDistanceMeters))
        _distanceBaseline = currentDistanceMeters;
}
...
function hideBlinkWarning() {
    const el = document.getElementById('blink-warning');
    if (el) el.style.display = 'none';
}
    ...
    if (playerRunning && !isPaused) {
        ...
        if (elapsedInWindowSec > blinkWindowSec / 2) {
            ...
            if (_blinkState.blinks.length < expectedSoFar) {
                if (now - _blinkState.warningShownAt > 12000) {
                    showBlinkWarning();
                    _blinkState.warningShownAt = now;
                }
            }
            const observedRate = (_blinkState.blinks.length / Math.max(1, elapsedInWindowSec)) * 60;
            if (_blinkState.minRateObserved == null || observedRate < _blinkState.minRateObserved)
                _blinkState.minRateObserved = observedRate;
        }
    }
}
function isBlinkResponseBlocked() {
    return performance.now() < _blinkResponseBlockUntil;
}
function areEyesClosedNow() {
    return blinkEnabled && _blinkState.isClosed === true;
}
...
function scheduleVoiceCountdown(durationMs, p) {
    ...
    if (durationMs - 5000 > 0) {
        timers.push(
            setTimeout(() => {
                if (!responsePhaseActive || isPaused) return;
...
function recalcSizesForNewDistance() {
    ...
    if (currentPlayingNodeId) {
        ...
        if (node && node.nodeType === 'STIMULUS') {
            ...
            if (svg) {
                svg.setAttribute('width', ns);
                svg.setAttribute('height', ns);
                svg.setAttribute('viewBox', `0 0 ${ns} ${ns}`);
            }
        }
        return;
    }
    ...
    if (readingViewportEl && readingViewportEl.style.display === 'block' && readingContentEl) {
        ...
        setTimeout(() => {
            ...
            scrollReadingToPage(readingPage);
        }, 60);
    }
}
async function processVideoFrame() {
            ...
            if (det && det.landmarks) {
                ...
                if (ipd > 0 && focalLengthPx) {
                    ...
                    if (distanceMax === null || currentDistanceMeters > distanceMax)
                        distanceMax = currentDistanceMeters;
                    distanceSum += currentDistanceMeters;
                    distanceCount++;
                    const now = performance.now();
                    if (now - distanceLastUpdate > 1000) {
                        distanceLog.push({ time: new Date().toISOString(), meters: currentDistanceMeters });
                        if (distanceLog.length > MAX_DISTANCE_LOG)
                            distanceLog.splice(0, distanceLog.length - MAX_DISTANCE_LOG);
                        distanceLastUpdate = now;
                    }
                    ...
                    if (playerRunning && !isPaused) {
                        ...
                        if (state === 'ok') {
                            _distanceOutOfBoundsSince = 0;
                            if (_distanceWarningKind) hideDistanceWarning();
                            _distanceRecalcScheduled = false;
                        } else {
                            if (_distanceWarningKind !== state) {
                                showDistanceWarning(state);
                                _distanceWarningKind = state;
                                _distanceOutOfBoundsSince = performance.now();
                                _distanceRecalcScheduled = false;
                            }
                            const el = (performance.now() - _distanceOutOfBoundsSince) / 1000;
                            if (
                                distanceRestoreTimeoutSec > 0 &&
                                el >= distanceRestoreTimeoutSec &&
                                !_distanceRecalcScheduled
                            ) {
                                _distanceRecalcScheduled = true;
                                recalcSizesForNewDistance();
                            }
                        }
                    }
                } else {
    ...
    if (cameraActive) videoFrameId = requestAnimationFrame(processVideoFrame);
    else videoFrameId = null;
}
...
function clearPeripheralLayer() {
    const layer = document.getElementById('peri-layer');
...
function startSingleStimAnimation(params) {
    ...
    if (!stimDisplay) return;
    const phases = buildGenericDynamicPhases(
        params.singleStimColor1,
        params.singleStimMidEnabled,
        params.singleStimColor3,
        params.singleStimColor2,
        params.singleStimReverse
    );
...
function startSingleBgAnimation(params) {
    ...
    if (!stimArea) return;
    const phases = buildGenericDynamicPhases(
        params.singleBgColor1,
        params.singleBgMidEnabled,
        params.singleBgColor3,
        params.singleBgColor2,
        params.singleBgReverse
    );
    const phaseCount = phases.length;
    if (phaseCount === 0) return;
...
function createNewNode(type, x, y) {
    ...
    if (type === 'READING') {
        node = {
            ...
            readingFontWeight: 'normal',
            readingAcuity: 1.0,
            readingDistance: readingDistance || 1,
            readingPPI: screenPPI || 96,
            readingTextColor: { r: 0, g: 0, b: 0 },
            readingBgMode: 'solid',
            readingBgColor: { r: 255, g: 255, b: 255 },
            readingSplitLeftWidthPercent: 50,
            readingSplitLeftColor: { r: 0, g: 0, b: 0 },
            readingSplitRightColor: { r: 255, g: 255, b: 255 },
            readingGradientMidEnabled: true,
            readingGradientLeftColor: { r: 255, g: 255, b: 255 },
            readingGradientMidColor: { r: 204, g: 204, b: 204 },
            readingGradientMidPosition: 50,
            readingGradientRightColor: { r: 0, g: 0, b: 0 },
            readingDynamicMode: 'simple',
            readingDynamicReverse: false,
            readingDynamicMidEnabled: false,
            readingDynamicStartColor: { r: 255, g: 0, b: 0 },
            readingDynamicMidColor: { r: 255, g: 255, b: 0 },
            readingDynamicEndColor: { r: 0, g: 0, b: 255 },
            readingDynamicDuration: 10000,
            readingDynamicLoop: false,
            readingDynamicStep: 0.1,
            readingDynamicStepDuration: 1000,
            readingDynamicShowLabel: true,
            duration: 60000,
            isActive: true,
            isStart: false
        };
        ...
        node = {
            ...
            singleGridX: 3,
            singleGridY: 3,
            singleGridShowLines: false,
            singleGridRandomCell: true,
            singleGridAvoidRepeat: true,
            singleGridFixedRow: 0,
            singleGridFixedCol: 0,
            singleGridCells: [],
            singleStimDynamicEnabled: false,
            singleStimColor1: { r: 255, g: 0, b: 0 },
            singleStimMidEnabled: false,
            singleStimColor3: { r: 255, g: 255, b: 0 },
            singleStimColor2: { r: 0, g: 0, b: 255 },
            singleStimDuration: 10000,
            singleStimLoop: false,
            singleStimReverse: false,
            singleBgDynamicEnabled: false,
            singleBgColor1: { r: 255, g: 0, b: 0 },
            singleBgMidEnabled: false,
            singleBgColor3: { r: 255, g: 255, b: 0 },
            singleBgColor2: { r: 0, g: 0, b: 255 },
            singleBgDuration: 10000,
            singleBgLoop: false,
            singleBgReverse: false,
            singleCircleEnabled: false,
            circleInnerEnabled: true,
            circleInnerRadiusPct: 40,
            circleInnerColor1: { r: 255, g: 0, b: 0 },
            circleInnerMidEnabled: false,
            circleInnerColor3: { r: 255, g: 255, b: 0 },
            circleInnerColor2: { r: 0, g: 0, b: 255 },
            circleInnerDuration: 10000,
            circleInnerLoop: true,
            circleInnerReverse: false,
            circleOuterEnabled: true,
            circleOuterColor1: { r: 0, g: 255, b: 0 },
            circleOuterMidEnabled: false,
            circleOuterColor3: { r: 0, g: 255, b: 255 },
            circleOuterColor2: { r: 0, g: 128, b: 255 },
            circleOuterDuration: 10000,
            circleOuterLoop: true,
            circleOuterReverse: false,
            periEnabled: false,
            periCount: 4,
            periAcuity: 0.3,
            periRadiusMinPct: 60,
            periRadiusMaxPct: 90,
            periColor: { r: 0, g: 255, b: 100 },
            periMotion: 'static',
            periSpeed: 0.3,
            periRandomAngles: true,
            dfEnabled: false,
            dfCenterRadiusMm: 13,
            dfCenterBg: { r: 204, g: 0, b: 0 },
            dfStimColor: { r: 0, g: 0, b: 0 },
            dfPeriBg: { r: 0, g: 71, b: 171 },
            dfPeriBlur: 0,
            blinkEnabled: false,
...
function deleteNode(id) {
    ...
    for (const [k, els] of connectionElements)
        if (k.includes(id)) {
            els.line.remove();
            els.arrow.remove();
            connectionElements.delete(k);
        }
...
function renderGraph() {
    ...
    connections.forEach((c) => {
        ...
        if (!connectionElements.has(k)) {
            ...
            if (line && arrow) {
                connectionElements.set(k, { line, arrow });
...
function clearTempLine() {
    ...
    if (tempLineMoveHandler) {
        canvas.removeEventListener('mousemove', tempLineMoveHandler);
        tempLineMoveHandler = null;
    }
}
...
function createNodeElement(node) {
    ...
    startCb.className = 'start-checkbox';
    startCb.checked = node.isStart === true;
    ...
    if (node.nodeType === 'READING') {
        ...
        b.className = 'node-reading-book';
        info.appendChild(b);
        const d = document.createElement('div');
        d.className = 'node-reading-sub';
        info.appendChild(d);
        const v = document.createElement('div');
        v.className = 'node-reading-sub';
        info.appendChild(v);
        const f = document.createElement('div');
        f.className = 'node-reading-sub';
        info.appendChild(f);
        content.appendChild(info);
        fillReadingNodeInfo(b, d, v, f, node);
        ...
        info.className = 'node-info';
        const m = document.createElement('div');
        m.className = 'node-compare-sub';
        info.appendChild(m);
        const g = document.createElement('div');
        g.className = 'node-compare-sub';
        info.appendChild(g);
        const s = document.createElement('div');
        s.className = 'node-compare-sub';
        info.appendChild(s);
        const t = document.createElement('div');
        t.className = 'node-compare-sub';
        info.appendChild(t);
        content.appendChild(info);
        fillCompareNodeInfo(m, g, s, t, node);
    } else {
        const p = document.createElement('div');
        p.className = 'node-preview-area';
        ...
        a.className = 'node-acuity';
        info.appendChild(a);
        const mm = document.createElement('div');
        mm.className = 'node-size-mm';
        info.appendChild(mm);
        const px = document.createElement('div');
        px.className = 'node-size-px';
        info.appendChild(px);
        const dst = document.createElement('div');
        dst.className = 'node-distance';
        info.appendChild(dst);
        const srs = document.createElement('div');
        srs.className = 'node-inherit';
        srs.style.color = '#a855f7';
        info.appendChild(srs);
        const dyn = document.createElement('div');
        dyn.className = 'node-inherit';
        dyn.style.color = '#f59e0b';
        info.appendChild(dyn);
        content.appendChild(info);
        fillStimulusNodeInfo(a, mm, px, dst, srs, dyn, node);
    }
...
function fillReadingNodeInfo(b, d, v, f, node) {
    ...
    if (v) {
        v.textContent = `V = ${(node.readingAcuity || 1.0).toFixed(1)}`;
        v.style.color = '#64748b';
    }
    if (f) {
        f.textContent = node.readingFontFamily || 'Segoe UI';
...
function updateNodeElement(el, node) {
    ...
    const sc = el.querySelector('.start-checkbox');
    if (sc) sc.checked = node.isStart === true;
    if (node.nodeType === 'READING') {
        const p = el.querySelector('.node-preview-area');
        if (p) p.style.background = '#0b0b14';
        const b = el.querySelector('.node-reading-book');
        const subs = el.querySelectorAll('.node-reading-sub');
    ...
    const a = el.querySelector('.node-acuity'),
        mm = el.querySelector('.node-size-mm'),
        px = el.querySelector('.node-size-px');
    const dst = el.querySelector('.node-distance');
    const inh = el.querySelectorAll('.node-inherit');
...
function startConnection(sourceId, isLoop) {
    ...
    tempLineElement.style.cssText =
        'position:absolute;transform-origin:top left;z-index:4;pointer-events:none;height:0;border-top:3px dashed #f59e0b;';
...
function createConnectionElements(conn) {
    const fn = getNode(conn.fromId),
        tn = getNode(conn.toId);
    if (!fn || !tn) return { line: null, arrow: null };
    if (conn.fromId === conn.toId) {
        const cx = fn.x + fn.width / 2,
            cy = fn.y + fn.height / 2,
            ls = 30;
    ...
    if (!isLoop) line.style.background = color;
    else line.style.borderTopColor = color;
...
function showConnectionMenu(e, conn) {
    ...
    sL.style.cssText = 'display:flex;align-items:center;gap:5px;margin-bottom:4px;';
    const sC = document.createElement('input');
    sC.type = 'checkbox';
    sC.checked = conn.inheritSize === true;
    sC.addEventListener('change', (ev) => {
        conn.inheritSize = ev.target.checked;
        requestRenderGraph();
    });
    sL.appendChild(sC);
    ...
    const spC = document.createElement('input');
    spC.type = 'checkbox';
    spC.checked = conn.inheritSpeed === true;
    spC.addEventListener('change', (ev) => {
        conn.inheritSpeed = ev.target.checked;
        requestRenderGraph();
    });
    spL.appendChild(spC);
    ...
    if (conn.isLoop) {
        ...
        lci.type = 'number';
        lci.min = '1';
        lci.value = conn.loopLimit || 1;
        lci.style.width = '100%';
        lci.addEventListener('change', (ev) => {
            conn.loopLimit = parseInt(ev.target.value) || 1;
            requestRenderGraph();
        });
        lcl.appendChild(lci);
        menu.appendChild(lcl);
    }
...
async function applyTemplateToCanvas(templateId, position) {
    ...
    const y = position?.y ?? canvas.scrollTop + 150 + Math.random() * 150;
    if (tpl.kind === 'group' && tpl.payload?.nodes) {
        const idMap = new Map();
        const newNodes = tpl.payload.nodes.map((n) => {
        ...
        (tpl.payload.connections || []).forEach((c) => {
...
async function loadTemplatesFolder() {
    if (!HAS_FS_ACCESS) {
        if (templatesFolderInput) {
            templatesFolderInput.value = '';
            templatesFolderInput.click();
        }
        return;
    }
    try {
...
async function importTemplatesFromFolder(handle) {
    if (!handle) return;
...
async function refreshTemplatesIfOpen() {
    ...
    if (modal && modal.style.display === 'flex') await renderTemplatesList();
}
async function renderTemplatesList(filter = '') {
    ...
    filtered.forEach((tpl) => {
        ...
        item.innerHTML = `
            <div style="flex:1;min-width:0;">
                ...
                <div style="font-size:11px;color:#94a3b8;margin-bottom:2px;">${kindLabel}</div>
...
function openTemplatesModal() {
    let modalEl = document.getElementById('templates-modal');
    if (!modalEl) {
        ...
        document
            .getElementById('templates-close')
            .addEventListener('click', () => (modalEl.style.display = 'none'));
        modalEl.addEventListener('click', (e) => {
            if (e.target === modalEl) modalEl.style.display = 'none';
        });
        document
            .getElementById('templates-search')
            .addEventListener('input', (e) => renderTemplatesList(e.target.value));
        document.getElementById('templates-import-folder').addEventListener('click', loadTemplatesFolder);
        document.getElementById('templates-export-all').addEventListener('click', exportAllTemplates);
    }
    modalEl.style.display = 'flex';
    renderTemplatesList();
}
async function exportAllTemplates() {
    ...
    a.href = url;
    a.download = 'vissort-templates-' + new Date().toISOString().slice(0, 10) + '.json';
...
function updateInspector() {
    ...
    if (node.nodeType === 'READING') {
        updateReadingInspector(node);
        return;
    }
    if (node.nodeType === 'LOGIC_IF') {
        updateLogicInspector(node);
        return;
    }
    if (node.nodeType === 'COMPARE') {
        updateCompareInspector(node);
        return;
    }
...
function renderInspectorGridPreview() {
    ...
    let sel = [];
    try {
        sel = JSON.parse(p.dataset.selected || '[]');
    } catch (_) {
        sel = [];
    }
    sel = sel.filter((c) => c && c.row >= 0 && c.row < gy && c.col >= 0 && c.col < gx);
    p.style.gridTemplateColumns = `repeat(${gx}, 30px)`;
    p.innerHTML = '';
    for (let r = 0; r < gy; r++)
        for (let c = 0; c < gx; c++) {
            ...
            cell.dataset.row = r;
            cell.dataset.col = c;
            cell.dataset.selected = isSel ? '1' : '0';
            cell.addEventListener('click', () => {
                const was = cell.dataset.selected === '1';
                cell.dataset.selected = was ? '0' : '1';
                cell.style.background = was ? '#1a1a1a' : '#22c55e';
                const list = [];
                p.querySelectorAll('[data-selected="1"]').forEach((e2) =>
                    list.push({ row: parseInt(e2.dataset.row), col: parseInt(e2.dataset.col) })
                );
...
function updateStimulusInspector(node) {
    ...
    if (p) {
        ...
        renderInspectorGridPreview();
    }
    const ai = document.getElementById('inp-acuity'),
    ...
    if (pp) pp.addEventListener('input', upd);
    ...
    const sM = document.getElementById('inp-sti-mid-enabled'),
        sMR = document.getElementById('insp-sti-mid-row');
    if (sM && sMR)
        sM.addEventListener('change', () => {
            sMR.style.display = sM.checked ? 'block' : 'none';
        });
    const bM = document.getElementById('inp-bg-mid-enabled'),
        bMR = document.getElementById('insp-bg-mid-row');
    if (bM && bMR)
        bM.addEventListener('change', () => {
            bMR.style.display = bM.checked ? 'block' : 'none';
        });
    const gxI = document.getElementById('inp-grid-x'),
        gyI = document.getElementById('inp-grid-y');
...
function updateCompareInspector(node) {
    ...
    function renderGrid() {
        ...
        let sel = [];
        try {
            sel = JSON.parse(previewEl.dataset.selected || '[]');
        } catch (_) {
            sel = [];
        }
    ...
    function readCells() {
        const c = document.getElementById('insp-cmp-cells');
        if (!c) return [];
        const res = [];
        c.querySelectorAll('.cmp-cell-block').forEach((div) => {
            ...
            res.push({
                ...
                distance: dist,
                ppi,
                size: acuityToSizePx(V, dist, ppi),
                stimR: sc.r,
                stimG: sc.g,
                stimB: sc.b,
                bgR: bc.r,
...
function updateReadingInspector(node) {
    ...
    if (gmc)
        gmc.addEventListener('change', () => {
            document.getElementById('insp-rd-grad-mid-row').style.display = gmc.checked ? 'block' : 'none';
        });
}
...
function initInspectorEvents() {
    inspectorEl.addEventListener('click', (e) => {
        ...
        else if (e.target.id === 'inp-reading-choose-book') openBookPicker();
    });
}
function applyInspectorChanges() {
    ...
    if (!node) return;
    if (node.nodeType === 'READING') {
        applyReadingInspectorChanges(node);
        requestRenderGraph();
        updateInspector();
        return;
    }
    if (node.nodeType === 'LOGIC_IF') {
        ...
        if (n) node.name = n.value;
        requestRenderGraph();
        updateInspector();
        return;
    }
    if (node.nodeType === 'COMPARE') {
        applyCompareInspectorChanges(node);
        requestRenderGraph();
        updateInspector();
        return;
    }
...
function applyPeriInspectorChanges(node) {
    ...
    if (v('inp-peri-count'))
        node.periCount = Math.max(1, Math.min(12, parseInt(v('inp-peri-count').value) || 4));
    if (v('inp-peri-acuity'))
        node.periAcuity = Math.max(0.1, Math.min(1.0, parseFloat(v('inp-peri-acuity').value) || 0.3));
    if (v('inp-peri-rmin'))
        node.periRadiusMinPct = Math.max(20, Math.min(100, parseInt(v('inp-peri-rmin').value) || 60));
    if (v('inp-peri-rmax'))
        node.periRadiusMaxPct = Math.max(20, Math.min(100, parseInt(v('inp-peri-rmax').value) || 90));
    ...
    if (v('inp-peri-speed'))
        node.periSpeed = Math.max(0.02, Math.min(5, parseFloat(v('inp-peri-speed').value) || 0.3));
...
function applyDefocusInspectorChanges(node) {
    ...
    if (v('inp-df-radius-mm'))
        node.dfCenterRadiusMm = Math.max(3, Math.min(50, parseFloat(v('inp-df-radius-mm').value) || 13));
...
function applyBlinkAnimationInspectorChanges(node) {
    ...
    const t = v('inp-blink-target');
    if (t) node.blinkTarget = t.value;
    const cA = v('inp-blink-cA');
    if (cA) node.blinkColorA = hexToRgb(cA.value);
    const cB = v('inp-blink-cB');
    if (cB) node.blinkColorB = hexToRgb(cB.value);
    const iv = v('inp-blink-interval');
    if (iv) node.blinkIntervalMs = Math.max(50, Math.min(5000, parseInt(iv.value) || 500));
    const dt = v('inp-blink-duty');
    if (dt) node.blinkDuty = Math.max(0.05, Math.min(0.95, parseFloat(dt.value) || 0.5));
    const cn = v('inp-blink-count');
    if (cn) node.blinkCount = Math.max(0, parseInt(cn.value) || 0);
}
function applyStimulusInspectorChanges(node) {
    ...
    if (v('inp-bg')) {
        const c = hexToRgb(v('inp-bg').value);
        node.bgR = c.r;
        node.bgG = c.g;
        node.bgB = c.b;
    }
    ...
    if (prev) {
        try {
            ...
            const gx = node.singleGridX || 3,
                gy = node.singleGridY || 3;
            node.singleGridCells = cells.filter(
                (c) =>
                    c &&
                    typeof c.row === 'number' &&
                    typeof c.col === 'number' &&
    ...
    if (v('inp-sti-duration'))
        node.singleStimDuration = Math.max(200, (parseFloat(v('inp-sti-duration').value) || 10) * 1000);
    ...
    if (v('inp-bg-duration'))
        node.singleBgDuration = Math.max(200, (parseFloat(v('inp-bg-duration').value) || 10) * 1000);
    ...
    if (v('inp-ci-radius'))
        node.circleInnerRadiusPct = Math.max(5, Math.min(95, parseInt(v('inp-ci-radius').value) || 40));
    ...
    if (v('inp-ci-duration'))
        node.circleInnerDuration = Math.max(200, (parseFloat(v('inp-ci-duration').value) || 10) * 1000);
    ...
    if (v('inp-co-duration'))
        node.circleOuterDuration = Math.max(200, (parseFloat(v('inp-co-duration').value) || 10) * 1000);
    ...
    ['delay1', 'duration', 'response', 'delay2'].forEach((f) => {
        const ni = document.getElementById('inp-' + f),
            us = document.getElementById('inp-' + f + '-unit');
...
function applyCompareInspectorChanges(node) {
    ...
    if (v('inp-cmp-pairs'))
        node.pairsCount = Math.max(1, Math.min(20, parseInt(v('inp-cmp-pairs').value) || 2));
...
function applyReadingInspectorChanges(node) {
    ...
    if (rdEl) {
        const n = parseFloat(rdEl.value);
        if (!isNaN(n) && n > 0) {
            node.readingDistance = n;
            readingDistance = n;
            saveUserSettings();
        }
    }
    ...
    if (v('inp-rd-split-left-width'))
        node.readingSplitLeftWidthPercent = parseInt(v('inp-rd-split-left-width').value) || 50;
    if (v('inp-rd-split-left-color'))
        node.readingSplitLeftColor = hexToRgb(v('inp-rd-split-left-color').value);
    if (v('inp-rd-split-right-color'))
        node.readingSplitRightColor = hexToRgb(v('inp-rd-split-right-color').value);
    if (v('inp-rd-grad-mid-enabled')) node.readingGradientMidEnabled = v('inp-rd-grad-mid-enabled').checked;
    if (v('inp-rd-grad-left-color'))
        node.readingGradientLeftColor = hexToRgb(v('inp-rd-grad-left-color').value);
    if (v('inp-rd-grad-mid-color')) node.readingGradientMidColor = hexToRgb(v('inp-rd-grad-mid-color').value);
    if (v('inp-rd-grad-mid-pos'))
        node.readingGradientMidPosition = parseInt(v('inp-rd-grad-mid-pos').value) || 50;
    if (v('inp-rd-grad-right-color'))
        node.readingGradientRightColor = hexToRgb(v('inp-rd-grad-right-color').value);
    ...
    if (v('inp-rd-dyn-duration'))
        node.readingDynamicDuration = Math.max(
            200,
            (parseFloat(v('inp-rd-dyn-duration').value) || 10) * 1000
        );
    ...
    const durEl = v('inp-reading-duration'),
        durU = v('inp-reading-duration-unit');
    if (durEl && durU) {
        const val = parseFloat(durEl.value) || 0;
        node.duration = val > 0 ? unitToMs(val, durU.value) : 0;
    }
...
function openBookPicker() {
    const n = getNode(activeNodeId);
    if (!n || n.nodeType !== 'READING') return;
    renderBookPickerList();
    const f = document.getElementById('book-picker-file-name');
    if (f) f.textContent = '';
    const i = document.getElementById('book-picker-file');
    if (i) i.value = '';
...
function renderBookPickerList() {
    const c = document.getElementById('book-picker-list');
    if (!c) return;
    const node = getNode(activeNodeId),
    ...
    ids.sort((a, b) =>
        (window._books[a].name || '').toLowerCase().localeCompare((window._books[b].name || '').toLowerCase())
    );
    c.innerHTML = '';
    ids.forEach((id) => {
...
function applyBookSelection(bookId) {
    const node = getNode(activeNodeId);
...
function detachBookFromActiveNode() {
    const node = getNode(activeNodeId);
    if (!node || node.nodeType !== 'READING') return;
    node.bookId = null;
    node.bookName = '';
...
function applyReadingFontForNode(node) {
    if (!readingContentEl) return;
    const family = (node.readingFontFamily && node.readingFontFamily.trim()) || 'Segoe UI';
    ...
    readingContentEl.style.fontWeight = node.readingFontWeight || 'normal';
}
...
function changeAcuityByStep(delta) {
    if (!window._currentReadingNodeId) return;
    const node = getNode(window._currentReadingNodeId);
    if (!node || node.nodeType !== 'READING') return;
    const nV = Math.max(0.1, Math.min(2.0, Math.round((node.readingAcuity + delta) * 10) / 10));
    if (Math.abs(nV - node.readingAcuity) < 0.001) return;
    node.readingAcuity = nV;
    const d = currentDistanceMeters || node.readingDistance || readingDistance || 1;
    if (readingContentEl)
        readingContentEl.style.fontSize =
            acuityToFontSizePx(node.readingAcuity, d, node.readingPPI || screenPPI) + 'px';
    setTimeout(() => {
        readingTotalPages = calcReadingTotalPages();
        scrollReadingToPage(0);
    }, 60);
}
function updateReadingFontIndicator() {
    const f = document.getElementById('reading-font-input');
    if (f) f.value = readingFontFamily;
    const b = document.getElementById('reading-bold-toggle');
    if (b) b.classList.toggle('active', readingFontWeight === 'bold');
}
function saveCurrentReadingBookmarkSilently() {
    if (!window._currentReadingBookId) return;
    try {
        localStorage.setItem(
            bookmarkKeyFor(window._currentReadingBookId),
            JSON.stringify({ page: readingPage, updatedAt: new Date().toISOString() })
...
function buildPhysiologicalPhases(step) {
    const sd = Math.max(0.1, parseFloat(step) || 0.1);
    const sel = [];
    for (const p of PHYSIOLOGICAL_PHASES) {
        const r = p.diopters / sd;
        if (Math.abs(r - Math.round(r)) < 1e-6) sel.push(p);
    }
    const last = PHYSIOLOGICAL_PHASES[PHYSIOLOGICAL_PHASES.length - 1];
    if (sel.length === 0 || sel[sel.length - 1] !== last) sel.push(last);
    const ph = [];
    for (let i = 0; i < sel.length - 1; i++) {
        const a = sel[i],
            b = sel[i + 1];
        ph.push({
            from: { r: a.rgb[0], g: a.rgb[1], b: a.rgb[2] },
            to: { r: b.rgb[0], g: b.rgb[1], b: b.rgb[2] },
            metaFrom: a,
            metaTo: b
        });
    }
...
function buildDynamicPhases(p) {
    const m = p.dynamicMode || 'simple',
        rev = p.dynamicReverse === true;
    let base;
    if (m === 'physiological') base = buildPhysiologicalPhases(p.dynamicStep || 0.1);
    else if (m === 'rgb')
        base = [
        ...
        const A = p.dynamicStartColor || { r: 255, g: 0, b: 0 },
            B = p.dynamicEndColor || { r: 0, g: 0, b: 255 };
        base =
    ...
    if (rev)
        return base.concat(
            base
                .slice()
                .reverse()
                .map((ph) => ({ from: ph.to, to: ph.from, metaFrom: ph.metaTo, metaTo: ph.metaFrom }))
        );
    return base;
}
function updatePhysioIndicator(meta, p) {
    const i = document.getElementById('physio-indicator');
    if (!i) return;
    if (p && p.dynamicShowLabel === false) {
...
function startDynamicReadingBackground(p) {
    ...
    if (!readingViewportEl) return;
    const m = p.dynamicMode || 'simple',
        phases = buildDynamicPhases(p),
        cnt = phases.length;
    const rgb = m === 'rgb',
        phys = m === 'physiological',
        rev = p.dynamicReverse === true;
    const cycleMs = phys
        ? Math.max(100, p.dynamicStepDuration || 1000) * cnt
        : Math.max(100, p.dynamicDuration || 10000);
    const loop = p.dynamicLoop === true || rgb || rev || phys;
    const first = phases[0].from;
    readingViewportEl.style.background = `rgb(${first.r},${first.g},${first.b})`;
    if (phys) updatePhysioIndicator(phases[0].metaFrom, p);
    else updatePhysioIndicator(null, p);
    readingBgAnimStart = null;
    readingBgAnimPausedAt = null;
    function tick(now) {
        if (readingPaused) {
        ...
        if (t >= 1) {
            if (loop) {
                readingBgAnimStart += Math.floor(t) * cycleMs;
                el = Math.max(0, now - readingBgAnimStart);
                t = el / cycleMs;
            } else {
                const l = phases[cnt - 1].to;
                readingViewportEl.style.background = `rgb(${l.r},${l.g},${l.b})`;
                if (phys && phases[cnt - 1].metaTo) updatePhysioIndicator(phases[cnt - 1].metaTo, p);
        ...
        const pi = Math.max(0, Math.min(cnt - 1, Math.floor(t * cnt))),
            pp = Math.max(0, Math.min(1, t * cnt - pi));
        const ph = phases[pi];
        if (!ph) {
            readingBgAnimId = null;
            return;
...
function stopDynamicReadingBackground() {
    if (readingBgAnimId) {
...
function applyReadingBackground(p) {
    ...
    if (m === 'dynamic') {
        startDynamicReadingBackground(p);
        return;
    }
...
function collectFontList() {
    ...
    const dl = document.getElementById('reading-font-list');
    if (dl)
...
function renderFontPickerList(filter) {
    const c = document.getElementById('font-picker-list');
    if (!c) return;
    const fonts = collectFontList();
    ...
    list.forEach((name) => {
        const item = document.createElement('div');
        item.className = 'font-picker-item' + (name.toLowerCase() === cur ? ' active' : '');
        const l = document.createElement('span');
        l.textContent = name;
        l.style.fontFamily = `'${name.replace(/['"]/g, '')}', 'Segoe UI', sans-serif`;
        item.appendChild(l);
        ...
        item.addEventListener('click', () => {
            const node = getNode(window._currentReadingNodeId);
            if (node && node.nodeType === 'READING') {
                node.readingFontFamily = name;
                applyReadingFontForNode(node);
                requestRenderGraph();
            } else {
                readingFontFamily = name;
                if (readingContentEl)
                    readingContentEl.style.fontFamily = `'${name.replace(/['"]/g, '')}', 'Segoe UI', sans-serif`;
            }
            saveUserSettings();
            closeFontPicker();
            setTimeout(() => {
...
function openFontPicker() {
    const m = document.getElementById('font-picker-modal');
    if (!m) return;
    const s = document.getElementById('font-picker-search');
    if (s) s.value = '';
...
function hideStimulus() {
    ...
    stimDisplay.style.color = '';
    stimDisplay.style.backgroundColor = '';
    stimDisplay.style.background = '';
    stimArea.style.backgroundColor = '';
}
function updateCounters() {
    cntCompleted.textContent = completedSeries;
    cntSuccess.textContent = successfulSeries;
    cntFailed.textContent = failedSeries;
    cntSeriesCorrect.textContent = seriesCorrect;
    cntSeriesIncorrect.textContent = seriesIncorrect;
    cntSeriesNoAnswer.textContent = seriesNoAnswer;
}
...
function startAutoTraining() {
    ...
    noAnswerSeriesStreak = seriesStep = 0;
    lastDirection = null;
    currentSingleCell = { row: 0, col: 0 };
    currentCompareNode = null;
    readingCurrentNode = null;
    window._currentReadingBookId = null;
    window._currentReadingPage = 0;
    window._currentReadingNodeId = null;
    ...
    distanceLog = [];
    distanceMin = distanceMax = null;
    distanceSum = distanceCount = 0;
    currentDuration = 2550;
    currentStimColor = trainingNode.params.startStimColor
        ? { ...trainingNode.params.startStimColor }
        : { r: 0, g: 255, b: 0 };
    currentBgColor = trainingNode.params.startBgColor
        ? { ...trainingNode.params.startBgColor }
        : { r: 0, g: 0, b: 0 };
    updateCounters();
    ...
    if (tt === 'reading') {
        ...
        startReadingModeAuto();
    } else if (tt === 'compare') {
        compareMode = trainingNode.params.compareMode || 'direction';
        gridX = trainingNode.params.gridX || 3;
        gridY = trainingNode.params.gridY || 3;
        activeCells = trainingNode.params.activeCells || [];
        cellParams = trainingNode.params.cellParams || [];
        if (activeCells.length === 0)
            activeCells = [
                { row: 0, col: 0 },
                { row: 0, col: 1 }
            ];
        if (cellParams.length < activeCells.length) cellParams = activeCells.map(() => defaultCellParams());
        showNextComparison();
...
function playNextQueuedNode() {
    ...
    if (node.nodeType === 'READING') {
        playReadingNode(node);
        return;
    }
    if (node.nodeType === 'COMPARE') {
        playCompareNodeSeries(node);
        return;
    }
    ...
    lastDirection = null;
    currentSingleCell = { row: 0, col: 0 };
    nodeAcuityCurrent = node.stimAcuity || 1.0;
    updateCounters();
    hideStimulus();
...
function playNextCompareRound(node) {
    ...
    if (seriesStep >= (node.seriesSize || 6)) {
        finishCompareNodeSeries(node);
        return;
    }
    if (blinkLockShow && areEyesClosedNow()) {
        phaseTimers.push(
            setTimeout(() => {
                if (!playerRunning || isPaused) return;
                playNextCompareRound(node);
...
function finishCompareNodeSeries(node) {
    ...
    if (seriesNoAnswer === (node.seriesSize || 6)) noAnswerSeriesStreak++;
    ...
    if (node.singleCircleEnabled) {
        ...
        if (node.singleStimDynamicEnabled)
            startSingleStimAnimation({
                singleStimColor1: node.singleStimColor1,
                singleStimMidEnabled: node.singleStimMidEnabled,
                singleStimColor3: node.singleStimColor3,
                singleStimColor2: node.singleStimColor2,
                singleStimReverse: node.singleStimReverse,
                singleStimLoop: node.singleStimLoop,
                singleStimDuration: node.singleStimDuration
            });
        else stopSingleStimAnimation();
    }
    if (node.singleBgDynamicEnabled)
        startSingleBgAnimation({
            singleBgColor1: node.singleBgColor1,
            singleBgMidEnabled: node.singleBgMidEnabled,
            singleBgColor3: node.singleBgColor3,
            singleBgColor2: node.singleBgColor2,
            singleBgReverse: node.singleBgReverse,
            singleBgLoop: node.singleBgLoop,
            singleBgDuration: node.singleBgDuration
        });
...
function finishStimulusNodeSeries(node) {
    ...
    if (ok) {
        ...
        if (nodeAcuityCurrent < eA)
            nodeAcuityCurrent = Math.min(
                eA,
                Math.round((nodeAcuityCurrent + (node.acuityStep || 0.1)) * 10) / 10
            );
    } else {
        const sA = node.stimAcuity || 1.0;
        if (nodeAcuityCurrent > sA)
            nodeAcuityCurrent = Math.max(
                sA,
                Math.round((nodeAcuityCurrent - (node.acuityStep || 0.1)) * 10) / 10
            );
    }
...
function playReadingNode(node) {
    ...
    if (!readingViewportEl || !readingContentEl) {
        playIndex++;
        playNextQueuedNode();
        return;
    }
    ...
    let bid = node.bookId,
        ...
        inherit = false;
    if (!bid) {
        if (window._currentReadingBookId && window._books[window._currentReadingBookId]) {
            bid = window._currentReadingBookId;
            inherit = true;
        } else {
            const l = getLastBookId();
            if (l) {
                bid = l;
                inherit = true;
            }
    ...
    else if (node.continueFromBookmark && bid) {
        const s = localStorage.getItem(bookmarkKeyFor(bid));
        if (s) {
            try {
                page = JSON.parse(s).page || 0;
            } catch (_) {}
        }
    }
    window._currentReadingNodeId = node.id;
    window._currentReadingBookId = bid || null;
    ...
    if (!bk) {
        ...
        readingContentEl.innerHTML = ph
            ...
            .map((l) => `<p>${l}</p>`)
            .join('');
        readingContentEl.style.color = rgbToHex(
            node.readingTextColor?.r,
            node.readingTextColor?.g,
            node.readingTextColor?.b
        );
    ...
    setTimeout(() => {
        readingTotalPages = calcReadingTotalPages();
        scrollReadingToPage(page);
    }, 100);
    if (node.duration > 0) {
        startReadingNodeTimer(Math.ceil(node.duration / 1000));
        currentShowTimer = setTimeout(() => finishReadingNodeAuto(), node.duration);
        phaseTimers.push(currentShowTimer);
    } else window._readingNodeWaiting = true;
}
function finishReadingNodeAuto() {
...
function startReadingNodeTimer(sec) {
    stopReadingNodeTimer();
...
function stopReadingNodeTimer() {
    if (readingNodeTimerInterval) {
        clearInterval(readingNodeTimerInterval);
        readingNodeTimerInterval = null;
    }
    const el = document.getElementById('reading-timer');
    if (el) el.style.display = 'none';
}
...
function startReadingModeAuto() {
    ...
    const pars = text
        ...
        .filter((x) => x.length > 0);
    readingContentEl.innerHTML = pars.map((x) => `<p>${escapeHtml(x)}</p>`).join('');
    readingViewportEl.style.color = p.textColor
    ...
    currentShowTimer = setTimeout(() => {
        ...
        if (!lastResponse.answered) {
            ...
            window.Data.saveResult({
                session_id: currentSessionId,
                node_id: 'training_node',
                response_time_ms: lastResponse.reactionTimeMs,
                is_correct: lastResponse.isCorrect
            });
        }
        if (lastResponse.answered) {
            if (lastResponse.isCorrect) seriesCorrect++;
...
function finishSeries() {
    const th = trainingNode?.params?.seriesThreshold || getThreshold(trainingNode?.params?.seriesSize || 6);
    const ok = seriesCorrect >= th;
    if (seriesNoAnswer === (trainingNode?.params?.seriesSize || 6)) noAnswerSeriesStreak++;
    else noAnswerSeriesStreak = 0;
    completedSeries++;
    if (ok) successfulSeries++;
    else failedSeries++;
    if (ok) {
        if (currentAcuity < (trainingNode?.params?.endAcuity || 2.0))
            currentAcuity = Math.min(
                trainingNode?.params?.endAcuity || 2.0,
                Math.round((currentAcuity + (trainingNode?.params?.acuityStep || 0.1)) * 10) / 10
            );
    } else {
        if (currentAcuity > (trainingNode?.params?.startAcuity || 0.5))
            currentAcuity = Math.max(
                trainingNode?.params?.startAcuity || 0.5,
                Math.round((currentAcuity - (trainingNode?.params?.acuityStep || 0.1)) * 10) / 10
            );
    }
    const dCalc = currentDistanceMeters || generalDistance || trainingDistance;
    ...
    if (completedSeries >= (trainingNode?.params?.seriesCount || 5)) {
        ...
        const tt = trainingNode?.params?.trainingType || 'single';
        if (tt === 'compare') showNextComparison();
        else showNextStimulus();
    }
}
...
function showNextComparison() {
    ...
    const dur = cellParams[0]?.duration || currentDuration;
    currentShowTimer = setTimeout(() => {
        if (responsePhaseActive) {
            lastResponse = { answered: false, isCorrect: false };
            processComparisonAnswer(false);
...
function showDirectionComparison() {
    ...
    document.querySelectorAll('.btn-response[data-dir]').forEach((b) => (b.style.display = 'none'));
    document.querySelectorAll('.btn-response[data-answer]').forEach((b) => (b.style.display = 'flex'));
    responseButtons.style.display = 'flex';
}
function showFindSameComparison() {
    const pc = Math.max(
        1,
        Math.min(20, parseInt(currentCompareNode?.pairsCount || trainingNode?.params?.pairsCount || 2))
    );
    const need = pc * 2;
    if (activeCells.length < need) {
        const maxP = Math.floor(activeCells.length / 2);
        if (maxP < 1) {
            processComparisonAnswer(false);
            return;
        }
        return showFindSameComparisonInternal(maxP);
    }
    showFindSameComparisonInternal(pc);
}
...
function defaultCellParams() {
    return {
        size: currentSize,
        stimR: currentStimColor.r,
        stimG: currentStimColor.g,
        stimB: currentStimColor.b,
        bgR: currentBgColor.r,
        bgG: currentBgColor.g,
        bgB: currentBgColor.b,
...
function processComparisonAnswer(isCorrect) {
    ...
    if (currentCompareNode) {
        ...
        phaseTimers.push(
            setTimeout(() => {
                if (playerRunning && !isPaused) playNextCompareRound(n);
            }, n.delay2 || 1000)
        );
        return;
    }
    ...
    phaseTimers.push(
        setTimeout(() => {
            if (playerRunning && !isPaused) showNextComparison();
...
function handleDirectionAnswer(direction) {
    ...
    document.body.style.backgroundColor = ok ? '#0a0' : '#a00';
    setTimeout(() => (document.body.style.backgroundColor = '#111'), 200);
}
function handleCompareDirectionAnswer(answer) {
    if (!responsePhaseActive) return;
    if (isBlinkResponseBlocked()) return;
...
document.addEventListener('keydown', (e) => {
    if (readingViewportEl && readingViewportEl.style.display === 'block') {
        ...
        if (e.key === 'Escape' && window._readingNodeWaiting) {
            e.preventDefault();
            finishReadingNode();
            return;
        }
...
function exitTrainingFromPause() {
    pauseModal.style.display = 'none';
    stopPlayer();
}
...
function ensureSingleGridGeneratorUI() {
    ...
    block.id = 'gen-single-grid-block';
    block.style.cssText =
    ...
    const set = document.getElementById('gen-single-grid-settings');
    cb.addEventListener('change', () => {
        set.style.display = cb.checked ? 'block' : 'none';
    });
}
function ensureDistanceGeneratorUI() {
    const typeSel = document.getElementById('gen-training-type');
    if (!typeSel || document.getElementById('gen-distance-block')) return;
    const firstCol = typeSel.closest('.modal-col');
    if (!firstCol) return;
    const block = document.createElement('div');
    block.id = 'gen-distance-block';
    block.style.cssText =
...
function ensureBlinkGeneratorUI() {
    const typeSel = document.getElementById('gen-training-type');
    if (!typeSel || document.getElementById('gen-blink-block')) return;
    const firstCol = typeSel.closest('.modal-col');
    if (!firstCol) return;
    const block = document.createElement('div');
    block.id = 'gen-blink-block';
    block.style.cssText =
    ...
    document.getElementById('gen-blink-min-rate').addEventListener('change', (e) => {
    ...
    document.getElementById('gen-blink-window').addEventListener('change', (e) => {
        const v = parseFloat(e.target.value);
    ...
    document.getElementById('gen-blink-lock-show').addEventListener('change', (e) => {
        blinkLockShow = e.target.checked;
        saveUserSettings();
    });
}
function ensureCircleGeneratorUI() {
    ...
    container.appendChild(block);
    const cb = document.getElementById('gen-circle-enabled'),
        set = document.getElementById('gen-circle-settings');
    cb.addEventListener('change', () => {
        set.style.display = cb.checked ? 'block' : 'none';
    });
    const ciM = document.getElementById('gen-ci-mid-enabled'),
        ciMR = document.getElementById('gen-ci-mid-row');
    ciM.addEventListener('change', () => {
        ciMR.style.display = ciM.checked ? 'block' : 'none';
    });
    const coM = document.getElementById('gen-co-mid-enabled'),
        coMR = document.getElementById('gen-co-mid-row');
    coM.addEventListener('change', () => {
        coMR.style.display = coM.checked ? 'block' : 'none';
    });
}
...
function renderGridPreview() {
    ...
    const c = document.getElementById('grid-preview');
    c.style.gridTemplateColumns = `repeat(${gX}, 40px)`;
    c.innerHTML = '';
    if (selectedCells.length > gX * gY) selectedCells = selectedCells.slice(0, gX * gY);
    for (let row = 0; row < gY; row++)
        for (let col = 0; col < gX; col++) {
            ...
            el.dataset.row = row;
            el.dataset.col = col;
            if (selectedCells.some((sc) => sc.row === row && sc.col === col)) el.style.background = '#8b5cf6';
            el.addEventListener('click', () => {
                const i = selectedCells.findIndex((sc) => sc.row === row && sc.col === col);
                if (i === -1) selectedCells.push({ row, col });
                else selectedCells.splice(i, 1);
                renderGridPreview();
                generateCellParamsFields();
            });
            c.appendChild(el);
        }
}
function generateCellParamsFields() {
    const c = document.getElementById('cell-params-container');
    c.innerHTML = '';
...
function collectCompareParams() {
    const gX = parseInt(document.getElementById('gen-grid-x').value) || 3;
    const gY = parseInt(document.getElementById('gen-grid-y').value) || 3;
    const cm = document.getElementById('gen-compare-mode').value;
    const d = parseFloat(document.getElementById('gen-distance').value) || generalDistance || 1;
    const ppi = parseInt(document.getElementById('gen-ppi').value) || screenPPI || 96;
    const cp = [];
    document.querySelectorAll('#cell-params-container > div').forEach((div) => {
        const V = parseFloat(div.querySelector('.cell-acuity').value) || 1.0;
        const sc = hexToRgb(div.querySelector('.cell-stim-color').value);
        const bc = hexToRgb(div.querySelector('.cell-bg-color').value);
        const dur = parseInt(div.querySelector('.cell-duration').value) || 1000;
        cp.push({
            size: acuityToSizePx(V, d, ppi),
            acuity: V,
            distance: d,
            ppi,
            stimR: sc.r,
...
function buildModeParamsFromUI() {
    ...
    const gdEl = document.getElementById('gen-general-distance');
    if (gdEl) {
        const v = parseFloat(gdEl.value);
        if (!isNaN(v) && v > 0) {
            generalDistance = v;
            saveUserSettings();
        }
    }
    const rdEl = document.getElementById('gen-reading-distance');
    if (rdEl) {
        const v = parseFloat(rdEl.value);
        if (!isNaN(v) && v > 0) {
            readingDistance = v;
            saveUserSettings();
        }
    }
    const tiEl = document.getElementById('gen-tol-inc');
    if (tiEl) {
        const v = parseFloat(tiEl.value);
        if (!isNaN(v) && v >= 0 && v <= 100) {
            distanceToleranceIncreasePct = v;
            saveUserSettings();
        }
    }
    const tdEl = document.getElementById('gen-tol-dec');
    if (tdEl) {
        const v = parseFloat(tdEl.value);
        if (!isNaN(v) && v >= 0 && v <= 100) {
            distanceToleranceDecreasePct = v;
            saveUserSettings();
        }
    }
    const ttEl = document.getElementById('gen-tol-timeout');
    if (ttEl) {
        const v = parseFloat(ttEl.value);
        if (!isNaN(v) && v >= 0 && v <= 60) {
            distanceRestoreTimeoutSec = v;
            saveUserSettings();
        }
    }
    const p = {
        ...
        distanceMeters: generalDistance,
        readingDistance,
        distanceToleranceIncreasePct,
    ...
    if (tt === 'single') {
        ...
        p.singleStimDuration = unitToMs(
            safeVal('gen-single-stim-duration', 10, parseFloat),
            safeVal('gen-single-stim-duration-unit', 's')
        );
        ...
        p.singleBgDuration = unitToMs(
            safeVal('gen-single-bg-duration', 10, parseFloat),
            safeVal('gen-single-bg-duration-unit', 's')
        );
    ...
    if (tt === 'compare') {
        Object.assign(p, collectCompareParams());
        p.pairsCount = 2;
    }
    if (tt === 'reading') {
        ...
        p.dynamicDuration = unitToMs(
            safeVal('gen-reading-dynamic-duration', 10, parseFloat),
            safeVal('gen-reading-dynamic-duration-unit', 's')
        );
        p.dynamicLoop = safeChecked('gen-reading-dynamic-loop', false);
        p.dynamicStep = safeVal('gen-reading-dynamic-step', 0.1, parseFloat);
        p.dynamicStepDuration = unitToMs(
            safeVal('gen-reading-dynamic-step-duration', 1, parseFloat),
            safeVal('gen-reading-dynamic-step-duration-unit', 's')
        );
...
function modeParamsToNode(modeParams) {
    ...
    node.stimR = modeParams.startStimColor?.r ?? 255;
    node.stimG = modeParams.startStimColor?.g ?? 255;
    node.stimB = modeParams.startStimColor?.b ?? 255;
    ...
    node.singleGridCells = Array.isArray(modeParams.singleGridCells)
        ? modeParams.singleGridCells.slice()
        : [];
    ...
    node.delay1 = modeParams.delay1 || 1000;
    node.duration = modeParams.startDuration || 1000;
    node.response = modeParams.response || 1000;
    node.delay2 = modeParams.delay2 || 1000;
    node.stimSize = getNodeComputedSize(node);
...
function modeParamsToReadingNode(modeParams) {
    ...
    node.readingSplitLeftColor = modeParams.splitLeftColor;
    node.readingSplitRightColor = modeParams.splitRightColor;
    node.readingGradientMidEnabled = modeParams.gradientMidEnabled !== false;
    node.readingGradientLeftColor = modeParams.gradientLeftColor;
    node.readingGradientMidColor = modeParams.gradientMidColor;
    node.readingGradientMidPosition = modeParams.gradientMidPosition ?? 50;
    node.readingGradientRightColor = modeParams.gradientRightColor;
    ...
    node.readingDynamicStartColor = modeParams.dynamicStartColor;
    node.readingDynamicMidColor = modeParams.dynamicMidColor;
...
async function restoreTemplatesFolderHandle() {
    if (!HAS_FS_ACCESS) return false;
    try {
...
async function scanScenariosFolder() {
...
async function saveGraph() {
    ...
    try {
        ...
        if (HAS_FS_ACCESS && selectedFolderHandle) {
            try {
                const ok = await window.Data.ensurePermission(selectedFolderHandle, 'readwrite');
                if (ok) {
...
function loadGraph(file) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (data.scenarioKey && typeof data.scenarioKey === 'string')
                window._currentScenarioKey = data.scenarioKey;
            else window._currentScenarioKey = generateScenarioKey();
            if (data.distances) {
                if (typeof data.distances.general === 'number') generalDistance = data.distances.general;
                if (typeof data.distances.reading === 'number') readingDistance = data.distances.reading;
                if (typeof data.distances.incTol === 'number')
                    distanceToleranceIncreasePct = data.distances.incTol;
                if (typeof data.distances.decTol === 'number')
                    distanceToleranceDecreasePct = data.distances.decTol;
                if (typeof data.distances.timeout === 'number')
                    distanceRestoreTimeoutSec = data.distances.timeout;
            }
            if (data.blink) {
                if (typeof data.blink.enabled === 'boolean') blinkEnabled = data.blink.enabled;
                if (typeof data.blink.threshold === 'number') blinkThreshold = data.blink.threshold;
                if (typeof data.blink.minRate === 'number') blinkMinRate = data.blink.minRate;
                if (typeof data.blink.window === 'number') blinkWindowSec = data.blink.window;
                if (typeof data.blink.lockShow === 'boolean') blinkLockShow = data.blink.lockShow;
            }
            saveUserSettings();
            connections.forEach((c) => {
                if (c.inheritSize === undefined) c.inheritSize = false;
                if (c.inheritSpeed === undefined) c.inheritSpeed = false;
                delete c.inheritBook;
                delete c.inheritPosition;
            });
            nodes.forEach((node) => {
                if (
                    (node.nodeType === 'STIMULUS' || node.nodeType === 'DYNAMIC') &&
                    node.stimAcuity === undefined
                ) {
                    const d = generalDistance || 1,
                        ppi = screenPPI || 96;
                ...
                if (node.nodeType === 'STIMULUS' || node.nodeType === 'DYNAMIC') {
                    ...
                    if (node.circleInnerColor3 === undefined)
                        node.circleInnerColor3 = { r: 255, g: 255, b: 0 };
                    ...
                    if (node.circleOuterColor3 === undefined)
                        node.circleOuterColor3 = { r: 0, g: 255, b: 255 };
                    if (node.circleOuterColor2 === undefined)
                        node.circleOuterColor2 = { r: 0, g: 128, b: 255 };
                ...
                if (node.nodeType === 'READING') {
                    if (node.readingFontFamily === undefined) node.readingFontFamily = 'Segoe UI';
                    if (node.readingFontWeight === undefined) node.readingFontWeight = 'normal';
                    if (node.readingAcuity === undefined) node.readingAcuity = 1.0;
                    if (node.readingDistance === undefined) node.readingDistance = readingDistance || 1;
                    if (node.readingPPI === undefined) node.readingPPI = screenPPI || 96;
                    if (node.readingTextColor === undefined) node.readingTextColor = { r: 0, g: 0, b: 0 };
                    if (node.readingBgColor === undefined) node.readingBgColor = { r: 255, g: 255, b: 255 };
                    if (node.readingBgMode === undefined) node.readingBgMode = 'solid';
                    if (node.duration === undefined) node.duration = 60000;
                    if (node.autoSaveBookmark === undefined) node.autoSaveBookmark = true;
                    if (node.continueFromBookmark === undefined) node.continueFromBookmark = true;
                }
                if (node.nodeType === 'COMPARE') {
                    if (!node.compareMode) node.compareMode = 'direction';
                    if (node.pairsCount === undefined) node.pairsCount = 2;
                    if (!node.gridX) node.gridX = 3;
                    if (!node.gridY) node.gridY = 3;
                    if (!Array.isArray(node.activeCells) || node.activeCells.length < 2)
                        node.activeCells = [
                            { row: 0, col: 0 },
                            { row: 0, col: 1 }
                        ];
                    if (!Array.isArray(node.cellParams)) node.cellParams = [];
                    while (node.cellParams.length < node.activeCells.length)
                        node.cellParams.push(defaultCompareCellParams());
                    if (node.seriesCount === undefined) node.seriesCount = 5;
                    if (node.seriesSize === undefined) node.seriesSize = 6;
                    if (node.seriesThreshold === undefined) node.seriesThreshold = 4;
    ...
    try {
        ...
        list.querySelectorAll('.btn-load-scenario').forEach((btn) => {
            btn.addEventListener('click', async () => {
                ...
                const graph = sc.params?.graph || sc.params;
                if (graph?.nodes) {
                    nodes = graph.nodes;
                    connections = graph.connections || [];
                    window._books = graph.books || {};
                    if (sc.params?.scenarioKey) window._currentScenarioKey = sc.params.scenarioKey;
...
function showAuthModal(mode) {
    ...
    const t = document.getElementById('auth-title'),
        nl = document.getElementById('auth-name-label'),
        ni = document.getElementById('auth-name'),
...
function init() {
    ...
    document.getElementById('scenario-time-close')?.addEventListener('click', () => {
        document.getElementById('scenario-time-modal').style.display = 'none';
    });
    document
        .getElementById('screen-calib-cancel')
        ?.addEventListener(
            'click',
            () => (document.getElementById('screen-calib-modal').style.display = 'none')
        );
    ...
    document.getElementById('book-picker-file')?.addEventListener('change', async (e) => {
        const f = e.target.files[0];
        if (f) await handleBookPickerFileUpload(f);
        e.target.value = '';
    });
    document.getElementById('book-picker-modal')?.addEventListener('click', (e) => {
        if (e.target.id === 'book-picker-modal') closeBookPicker();
    });
    ...
    document.getElementById('reading-font-input')?.addEventListener('change', (e) => {
        const n = (e.target.value || '').trim();
        if (!n) return;
        const node = getNode(window._currentReadingNodeId);
        if (node && node.nodeType === 'READING') {
            node.readingFontFamily = n;
            applyReadingFontForNode(node);
            requestRenderGraph();
        } else {
            readingFontFamily = n;
            if (readingContentEl)
                readingContentEl.style.fontFamily = `'${n.replace(/['"]/g, '')}', 'Segoe UI', sans-serif`;
        }
        saveUserSettings();
        setTimeout(() => {
            setupReadingColumns();
    ...
    document
        .getElementById('font-picker-search')
        ?.addEventListener('input', (e) => renderFontPickerList(e.target.value));
    document.getElementById('font-picker-modal')?.addEventListener('click', (e) => {
        if (e.target.id === 'font-picker-modal') closeFontPicker();
    });
    document.getElementById('reading-font-sivtsev')?.addEventListener('click', () => {
        const node = getNode(window._currentReadingNodeId);
        if (node && node.nodeType === 'READING') {
            node.readingFontFamily = 'Sivtsev';
            applyReadingFontForNode(node);
            requestRenderGraph();
        } else {
            readingFontFamily = 'Sivtsev';
            if (readingContentEl) readingContentEl.style.fontFamily = `'Sivtsev', 'Segoe UI', sans-serif`;
        }
        const i = document.getElementById('reading-font-input');
        if (i) i.value = 'Sivtsev';
        saveUserSettings();
        setTimeout(() => {
            setupReadingColumns();
            readingTotalPages = calcReadingTotalPages();
    ...
    const fontFileBtn = document.getElementById('reading-font-file');
    const fontFileInput = document.getElementById('reading-font-file-input');
    if (fontFileBtn && fontFileInput) {
        ...
        fontFileInput.addEventListener('change', async (e) => {
            ...
            try {
                ...
                const familyName =
                    f.name.replace(/\.[^.]+$/, '').replace(/[^a-zA-Z0-9\u0400-\u04FF_ -]/g, '_') ||
                    'CustomFont';
                const font = new FontFace(familyName, buf);
                await font.load();
                document.fonts.add(font);
                const node = getNode(window._currentReadingNodeId);
                if (node && node.nodeType === 'READING') {
                    node.readingFontFamily = familyName;
                    applyReadingFontForNode(node);
                    requestRenderGraph();
                } else {
                    readingFontFamily = familyName;
                    if (readingContentEl)
                        readingContentEl.style.fontFamily = `'${familyName.replace(/['"]/g, '')}', 'Segoe UI', sans-serif`;
                }
                const i = document.getElementById('reading-font-input');
                if (i) i.value = familyName;
                saveUserSettings();
                setTimeout(() => {
                    setupReadingColumns();
                    readingTotalPages = calcReadingTotalPages();
    ...
    document.getElementById('reading-font-system')?.addEventListener('click', async () => {
        ...
        try {
            ...
            const fams = [...new Set(fonts.map((f) => f.family))].sort((a, b) => a.localeCompare(b, 'ru'));
            const list = document.getElementById('reading-font-list');
            list.innerHTML = '';
            ...
            const ordered = [
                ...pref.filter((p) => fams.includes(p)),
                ...fams.filter((f) => !pref.includes(f) && f.toLowerCase() !== 'sivtsev')
            ];
    ...
    document.getElementById('reading-bold-toggle')?.addEventListener('click', () => {
        const node = getNode(window._currentReadingNodeId);
        if (node && node.nodeType === 'READING') {
            node.readingFontWeight = node.readingFontWeight === 'bold' ? 'normal' : 'bold';
            applyReadingFontForNode(node);
            requestRenderGraph();
        } else {
            readingFontWeight = readingFontWeight === 'bold' ? 'normal' : 'bold';
            if (readingContentEl) readingContentEl.style.fontWeight = readingFontWeight;
        }
        const b = document.getElementById('reading-bold-toggle');
        if (b) {
            const isB = (node ? node.readingFontWeight : readingFontWeight) === 'bold';
            b.classList.toggle('active', isB);
        }
        saveUserSettings();
        setTimeout(() => {
            setupReadingColumns();
    ...
    if (readingFileInput) {
        readingFileInput.addEventListener('change', async (e) => {
            const f = e.target.files[0];
            if (!f) return;
    ...
    btnGenerator.addEventListener('click', () => {
        ...
        const dgX = parseInt(document.getElementById('gen-grid-x').value) || 3,
            dgY = parseInt(document.getElementById('gen-grid-y').value) || 3;
        if (dgX * dgY >= 2) {
        ...
        renderGridPreview();
        generateCellParamsFields();
        document.getElementById('gen-threshold').value = getThreshold(
            parseInt(document.getElementById('gen-series-size').value)
        );
        const p = document.getElementById('gen-ppi');
        if (p) p.value = screenPPI;
        if (readingFileName) readingFileName.textContent = '';
        const gdEl = document.getElementById('gen-general-distance');
        if (gdEl) gdEl.value = generalDistance;
        const rdEl = document.getElementById('gen-reading-distance');
        if (rdEl) rdEl.value = readingDistance;
        const tiEl = document.getElementById('gen-tol-inc');
        if (tiEl) tiEl.value = distanceToleranceIncreasePct;
        const tdEl = document.getElementById('gen-tol-dec');
        if (tdEl) tdEl.value = distanceToleranceDecreasePct;
        const ttEl = document.getElementById('gen-tol-timeout');
        if (ttEl) ttEl.value = distanceRestoreTimeoutSec;
        const beEl = document.getElementById('gen-blink-enabled');
        if (beEl) beEl.checked = blinkEnabled;
        const bmrEl = document.getElementById('gen-blink-min-rate');
        if (bmrEl) bmrEl.value = blinkMinRate;
        const bwEl = document.getElementById('gen-blink-window');
        if (bwEl) bwEl.value = blinkWindowSec;
        const blsEl = document.getElementById('gen-blink-lock-show');
        if (blsEl) blsEl.checked = blinkLockShow;
    ...
    document
        .getElementById('gen-reading-gradient-mid-enabled')
        ?.addEventListener('change', updateGradientMidVisibility);
    document
        .getElementById('gen-reading-dynamic-mid-enabled')
        ?.addEventListener('change', updateDynamicMidVisibility);
    document
        .getElementById('gen-reading-dynamic-mode')
        ?.addEventListener('change', updateDynamicModeVisibility);
    document.getElementById('gen-reading-dynamic-step')?.addEventListener('change', updatePhysioListHint);
    document
        .getElementById('gen-single-stim-dynamic-enabled')
        ?.addEventListener('change', updateSingleStimDynamicVisibility);
    document
        .getElementById('gen-single-stim-mid-enabled')
        ?.addEventListener('change', updateSingleStimMidVisibility);
    document
        .getElementById('gen-single-bg-dynamic-enabled')
        ?.addEventListener('change', updateSingleBgDynamicVisibility);
    document
        .getElementById('gen-single-bg-mid-enabled')
        ?.addEventListener('change', updateSingleBgMidVisibility);
    ...
    function updateBgModeVisibility() {
        ...
        const s = document.getElementById('reading-solid-settings'),
            sp = document.getElementById('reading-split-settings');
        const g = document.getElementById('reading-gradient-settings'),
            d = document.getElementById('reading-dynamic-settings');
    ...
    function updatePhysioListHint() {
        ...
        for (const p of PHYSIOLOGICAL_PHASES) {
            const r = p.diopters / sd;
            if (Math.abs(r - Math.round(r)) < 1e-6) c++;
        }
        const last = PHYSIOLOGICAL_PHASES[PHYSIOLOGICAL_PHASES.length - 1];
    ...
    document
        .getElementById('calibrate-cancel')
        .addEventListener(
            'click',
            () => (document.getElementById('calibration-modal').style.display = 'none')
        );
    ...
    document
        .getElementById('btn-library-close')
        .addEventListener('click', () => (document.getElementById('library-modal').style.display = 'none'));
    ...
    document
        .getElementById('btn-users-close')
        .addEventListener('click', () => (document.getElementById('users-modal').style.display = 'none'));
    ...
    (async () => {
        ...
        if (templatesFolderHandle) {
            try {
                await importTemplatesFromFolder(templatesFolderHandle);
            } catch (_) {}
        }
    })();
