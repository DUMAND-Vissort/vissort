...
async function openHistory(){
    ...
    const rows = Object.entries(groups).slice(0,30).map(([sid,g])=>{
...
function updateCounters(){
    ...
    cntAcuity.textContent = (p.nodeType === 'READING' || p.trainingType === 'reading') ? '—' : (acuityVal || 1).toFixed(1);
...
function startSingleStimAnimation(p){
    ...
    const cycMs=total; const loop=p.singleStimLoop===true;
    ...
    function tick(now){
        ...
        let t=el/cycMs;
        if(t>=1){
            if(loop){singleStimAnimStart+=Math.floor(t)*cycMs;el=Math.max(0,now-singleStimAnimStart);t=el/cycMs;}
...
function startSingleBgAnimation(p){
    ...
    const cycMs=total; const loop=p.singleBgLoop===true;
    const first=ph[0].from;stimArea.style.backgroundColor=`rgb(${first.r},${first.g},${first.b})`;
    ...
    function tick(now){
        ...
        let t=el/cycMs;
        if(t>=1){
            if(loop){singleBgAnimStart+=Math.floor(t)*cycMs;el=Math.max(0,now-singleBgAnimStart);t=el/cycMs;}
            else{const l=ph[cnt-1].to;stimArea.style.backgroundColor=`rgb(${l.r},${l.g},${l.b})`;singleBgAnimId=null;return;}
        }
        t=Math.max(0,t);
        const pi=Math.max(0,Math.min(cnt-1,Math.floor(t*cnt)));
        const pp=Math.max(0,Math.min(1,(t*cnt)-pi));
...
function getPeripheralLayer(){
    ...
    if(!l){l=document.createElement('div');l.id='peri-layer';l.style.cssText='position:absolute;inset:0;pointer-events:none;z-index:6;overflow:hidden;';if(getComputedStyle(stimArea).position==='static')stimArea.style.position='relative';stimArea.appendChild(l);}
    return l;
}
...
function removeSingleGridLines(){stimArea.querySelectorAll('.single-grid-overlay').forEach(el=>el.remove());}
...
function pickSingleGridCell(gx,gy,rand,avoid,fx,fy,cells){
    gx=Math.max(1,parseInt(gx)||1);gy=Math.max(1,parseInt(gy)||1);
    let allowed=[];
    if(Array.isArray(cells)&&cells.length>0)allowed=cells.filter(c=>c&&typeof c.row==='number'&&typeof c.col==='number'&&c.row>=0&&c.row<gy&&c.col>=0&&c.col<gx);
    if(allowed.length===0)for(let r=0;r<gy;r++)for(let c=0;c<gx;c++)allowed.push({row:r,col:c});
    if(!rand){const wr=Math.max(0,Math.min(gy-1,parseInt(fx)||0));const wc=Math.max(0,Math.min(gx-1,parseInt(fy)||0));return allowed.find(c=>c.row===wr&&c.col===wc)||allowed[0];}
...
function evaluateDistance(){
    ...
    if(dev>upTol){
        if(camWarnKind!=='up'){camWarnKind='up';camIndicator.style.color='#ff6666';camIndicator.textContent='📏 Не отклоняйтесь';}
    } else if(dev<-dnTol){
        if(camWarnKind!=='down'){camWarnKind='down';camIndicator.style.color='#f59e0b';camIndicator.textContent='📏 Не приближайтесь';}
    } else {
        camWarnKind=null;camIndicator.style.color='#fff';
        camIndicator.textContent=`📏 ${curDistanceM.toFixed(2)} м`;
    }
}
function disableCamera(){
    ...
    camActive=false;camIndicator.style.display='none';
}
...
function buildGraphQueue() {
    ...
    function visit(nodeId, connection = null, fromNodeId = null) {
        ...
        gConnections.filter(c => c.fromId === nodeId && c.isLoop && c.toId !== nodeId).forEach(loop => {
        ...
        gConnections.filter(c => c.fromId === nodeId && !c.isLoop && c.toId !== nodeId).forEach(conn => visit(conn.toId, conn, nodeId));
    }
...
function playNextGraphNode() {
    ...
    if (gIndex >= gQueue.length) {
        stopPlayer();
        showStatus('Граф пройден', `Серий: ${completedSeries}`, 'Ещё раз', () => { hideStatus(); startPlayer(); });
        return;
    }
    ...
    phaseTimers.push(setTimeout(() => { if (playerRunning && !isPaused) playGraphStimulus(node); }, node.delay1 || 0));
}
...
function playGraphStimulus(node) {
    ...
    currentShowTimer = setTimeout(() => {
        ...
        phaseTimers.push(setTimeout(() => { if (playerRunning && !isPaused) playGraphStimulus(node); }, node.delay2 || 1000));
    }, sd);
...
function finishGraphStimulusSeries(node) {
    ...
    if (completedSeries >= (node.seriesCount || 5)) { gIndex++; playNextGraphNode(); return; }
    phaseTimers.push(setTimeout(() => { if (playerRunning && !isPaused) playGraphStimulus(node); }, node.delay2 || 500));
}
...
function playGraphCompare(node) {
    ...
    phaseTimers.push(setTimeout(() => { if (playerRunning && !isPaused) playGraphCompareRound(node); }, node.delay1 || 0));
}
...
function processGraphCompareAnswer(isCorrect) {
    ...
    if (!node) { gIndex++; playNextGraphNode(); return; }
    if (seriesStep >= (node.seriesSize || 6)) { finishGraphCompareSeries(node); return; }
    phaseTimers.push(setTimeout(() => { if (playerRunning && !isPaused) playGraphCompareRound(node); }, node.delay2 || 1000));
}
...
function finishGraphCompareSeries(node) {
    ...
    phaseTimers.push(setTimeout(() => { if (playerRunning && !isPaused) playGraphCompareRound(node); }, node.delay2 || 500));
}
...
function playGraphReading(node) {
    ...
    readingContentEl.style.color = node.readingTextColor ? rgbToHex(node.readingTextColor.r, node.readingTextColor.g, node.readingTextColor.b) : '#000';
...
function startPlayer(){
    ...
    if(tt === 'reading'){btnPlayerPause.disabled = true; startReading();}
    else if(tt === 'compare'){
...
function showNextStimulus(){
    ...
    if(p.dfEnabled){
        ...
        if(frame){stimDisplay.innerHTML='';stimDisplay.appendChild(frame);stimArea.style.backgroundColor=`rgb(${p.dfPeriBg.r},${p.dfPeriBg.g},${p.dfPeriBg.b})`;}
...
function defaultCellParams(){
    ...
    return {size:acuityToSizePx(1.0,d,ppi),stimR:255,stimG:255,stimB:255,bgR:0,bgG:0,bgB:0,duration:2000};
}
...
function processCompareAnswer(isCorrect){
    ...
    if(seriesStep>=(p.seriesSize||6)){finishCompareSeries();return;}
    phaseTimers.push(setTimeout(()=>{if(playerRunning&&!isPaused)showNextCompareRound();},p.delay2||1000));
}
...
responseButtons.addEventListener('click',(e)=>{
    ...
    if(btn.dataset.dir){
        ...
        const inCmp=compareMode==='direction'&&(userScenario?.params?.trainingType==='compare'||graphActive);
        if(inCmp){
...
document.addEventListener('keydown',(e)=>{
    ...
    if(map[e.key]){
        ...
        const inCmp=compareMode==='direction'&&(userScenario?.params?.trainingType==='compare'||graphActive);
        if(inCmp){
            if(e.key==='ArrowLeft'){graphActive?handleGraphCompareAnswer(true):handleCompareAnswer(true);}
            else if(e.key==='ArrowRight'){graphActive?handleGraphCompareAnswer(false):handleCompareAnswer(false);}
        } else {
...
function startReading(){
    ...
    const pars=text.split(/\n+/).map(x=>x.trim()).filter(x=>x.length>0);
    readingContentEl.innerHTML=pars.map(x=>`<p>${escapeHtml(x)}</p>`).join('');
    readingContentEl.style.color=p.textColor?`rgb(${p.textColor.r},${p.textColor.g},${p.textColor.b})`:'#000';
...
function startReadingDynamicBg(p){
    ...
    const phases=rev?base.concat(base.slice().reverse().map(ph=>({from:ph.to,to:ph.from}))):base;
...
function finishReading(){
    ...
    updateCounters();
    stopPlayer();
    showStatus('Чтение завершено','Тренировка окончена.','Ещё раз',()=>{hideStatus();startPlayer();});
}
...
function showFinishedReport(){
    ...
    showStatus('Готово!',txt,'Ещё раз',()=>{hideStatus();startPlayer();});
}
...
function resumeTraining(){
    ...
    isPaused=false;noAnswerSeriesStreak=0;btnPlayerPause.disabled=false;
    if(graphActive){
        const node=gGetNode(gCurrentNodeId);
        if(node){
